...
function __booleanAssignFaceData(geometry, sourceMeta, debugLog, options: any = {}) {
  try {
  ...
  try {
    ...
    if (restoredIdToFaceName.size > 0) {
      ...
      rebuilt._faceMetadata = new Map(
        ...
        }).filter(([faceName]) => rebuilt._faceNameToID.has(faceName)) as any
      );
...
function __booleanRestoreFaceTrackingFromSources(solid, debugLog, ...sourceSolids) {
  try {
    ...
    if (!sourceGeometries.length) {
      try { targetMeta.geometry.dispose(); } catch { /* ignore */ }
      return solid;
    }
    ...
    try {
      ...
      try { rebuilt.name = solid.name || rebuilt.name; } catch { /* ignore */ }
      try { rebuilt.owningFeatureID = solid.owningFeatureID || rebuilt.owningFeatureID; } catch { /* ignore */ }
      return rebuilt;
    } finally {
      try { targetMeta.geometry.dispose(); } catch { /* ignore */ }
      for (const entry of sourceGeometries) {
        try { entry?.geometry?.dispose?.(); } catch { /* ignore */ }
      }
    }
  } catch (err) {
  ...
  try {
    for (const scale of attemptScales) {
      ...
      try {
        ...
        try { workingGeom.dispose(); } catch { /* ignore */ }
        continue;
      }
      ...
      if (!faceData) {
        try { repairedGeom.dispose(); } catch { /* ignore */ }
        continue;
      }
    ...
    try { baseGeom.dispose(); } catch { /* ignore */ }
  }
...
function __booleanMeshMergeUnion(baseSolid, toolSolid, eps, debugLog) {
  ...
  if (!srcA || !srcB) {
    try { srcA?.geometry?.dispose?.(); } catch { /* ignore */ }
    try { srcB?.geometry?.dispose?.(); } catch { /* ignore */ }
    return null;
  }
  ...
  if (!posA || !posB || !idxA || !idxB) {
    try { geomA.dispose(); } catch { /* ignore */ }
    try { geomB.dispose(); } catch { /* ignore */ }
    return null;
  }
...
export async function applyBooleanOperation(partHistory, baseSolid, booleanParam, featureID): Promise<any> {
  try {
    ...
    if (
      ...
      try {
        ...
        try { result.name = inheritedName || fallbackName; } catch (_) { /* ignore */ }
    ...
    if (op === 'SUBTRACT') {
      ...
      const preCleanLocal = (solid, eps) => {
        try { if (typeof solid.setEpsilon === 'function') solid.setEpsilon(eps); } catch { /* ignore */ }
        try { if (typeof solid.fixTriangleWindingsByAdjacency === 'function') solid.fixTriangleWindingsByAdjacency(); } catch { /* ignore */ }
      };
      ...
      const addResult = async (solid, target) => {
        ...
        try { solid.name = finalName; } catch (_) { /* ignore */ }
        try {
