...
export function free() {
    try {
        ...
        try { if (this._freeTimer) { clearTimeout(this._freeTimer); } } catch (_) { /* ignore timer cleanup failure */ }
        this._freeTimer = null;
        if (this._manifold) {
            try { if (typeof this._manifold.delete === 'function') this._manifold.delete(); } catch (_) { /* ignore manifold cleanup failure */ }
            this._manifold = null;
        }
        if (this._cppSolidCore) {
            try { if (typeof this._cppSolidCore.dispose === 'function') this._cppSolidCore.dispose(); } catch (_) { /* ignore native bridge cleanup failure */ }
