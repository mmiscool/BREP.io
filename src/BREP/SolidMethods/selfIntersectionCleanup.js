...
function invalidateGeometryCaches(solid) {
    solid._dirty = true;
...
function rebuildVertexKeyToIndex(solid) {
...
function modelTolerance(solid, explicit = null) {
...
function validateAuthoringArrays(solid) {
    const vp = solid._vertProperties;
...
function triangleAreaFromIndices(vp, a, b, c) {
...
function removeInvalidAndDuplicateTriangles(solid, tolerance = modelTolerance(solid)) {
    const vp = solid._vertProperties || [];
    const tv = solid._triVerts || [];
    const ids = solid._triIDs || [];
    const triCount = (tv.length / 3) | 0;
...
function compactUnusedVertices(solid) {
    const vp = solid._vertProperties || [];
    const tv = solid._triVerts || [];
    const vertexCount = (vp.length / 3) | 0;
...
function meshClosedAndCoherent(solid) {
    const tv = solid._triVerts || [];
    const triCount = (tv.length / 3) | 0;
    const edges = new Map();
...
function normalizeSplitOptions(options = false) {
    if (options === true || options === false) return { diagnostics: options === true };
    return options && typeof options === "object" ? { ...options } : {};
}

export function findSelfIntersections(options = {}) {
...
export function cleanupSelfIntersections(options = {}) {
