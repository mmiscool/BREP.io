...
function visualizeCacheSignature(solid, options: any) {
 ...
 */
export function visualize(options: any = {}) {
    // stack trace here
    //console.trace();
    ...
    for (const { faceName, triangles } of faces) {
        ...
        try { faceObj.owningFeatureID = this?.owningFeatureID || null; } catch { /* ignore owning feature tag assignment failure */ }
    ...
    if (showEdges) {
        if (!usedFallback) {
            ...
            if (!Array.isArray(polylines) || polylines.length === 0) {
                try { usedFallback = true; } catch { /* ignore fallback flag assignment failure */ }
            }
            for (const e of polylines) {
                ...
                try { g.computeBoundingSphere(); } catch { /* ignore line bounds failure */ }

                const edgeObj: any = new Edge(g);
                edgeObj.name = e.name;
                ...
                const addPolyline = (nameA, nameB, indices) => {
                    ...
                    for (const [u0] of adj.entries()) {
                        ...
                        if (poly.length >= 2) {
                            ...
                            try { edgeObj.computeLineDistances(); } catch { /* ignore line distance computation failure */ }
                            this.add(edgeObj);
                        }
                    }
                };
        ...
        if (Array.isArray(this._auxEdges) && this._auxEdges.length) {
            for (const aux of this._auxEdges) {
                ...
                try {
                    ...
                    const edgeMats: any = CADmaterials?.EDGE || {};
                        ...
                        if (!alreadyOverlay) {
                            ...
                            if (shared && typeof mat.clone === 'function') {
                                ...
                                if (cloned) {
                                    ...
                                    } catch { /* ignore resolution copy failure */ }
                                }
                            }
                            if ((cloned || !shared) && appliedMat) {
                                try { appliedMat.depthTest = false; } catch { /* ignore material depth-test assignment failure */ }
                                try { appliedMat.depthWrite = false; } catch { /* ignore material depth-write assignment failure */ }
                            }
                        }
                    }
                ...
                } catch { /* ignore aux edge material assignment failure */ }
    ...
    try {
        ...
        if (endpoints.size) {
            for (const [positionKey, position] of endpoints.entries()) {
                ...
                } catch { /* ignore vertex creation failure */ }
            }
        }
