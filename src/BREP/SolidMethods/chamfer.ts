...
function solidFromSnapshot(snapshot, name, SolidCtor) {
  ...
  try { solid.name = name; } catch { /* ignore name assignment failure */ }
  return solid;
}
 ...
 */
export async function chamfer(opts: any = {}) {
  ...
  if (unique.length === 0) {
    ...
    const c = this.clone();
    try { c.name = this.name; } catch { /* ignore name assignment failure */ }
    return c;
  }
  ...
  for (const entry of debugSnapshots) {
    ...
    if (debugSolid) {
      try { debugSolid.__debugChamferKind = String(entry?.kind || ''); } catch { /* ignore debug tag assignment failure */ }
      try { debugSolid.__debugChamferName = String(entry?.name || debugSolid?.name || ''); } catch { /* ignore debug tag assignment failure */ }
      debugChamferSolids.push(debugSolid);
    }
  }
  try { result.__debugChamferSolids = debugChamferSolids; } catch { /* ignore debug tag assignment failure */ }
  try { result.__chamferDirectionDecision = nativeResult?.directionDecision || null; } catch { /* ignore debug tag assignment failure */ }
