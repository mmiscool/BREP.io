...
export function _manifoldize() {
    ...
    try { if (this._freeTimer) { clearTimeout(this._freeTimer); } } catch { /* ignore stale timer cleanup failure */ }
    try {
        this._freeTimer = setTimeout(() => {
            try { this.free(); } catch { /* ignore scheduled cleanup failure */ }
        }, 60 * 1000);
    } catch { /* ignore timer scheduling failure */ }
    ...
    if (!this._dirty && this._manifold) {
        const __t1 = nowMs();
        try { if (debugMode) console.log(`[Solid] _manifoldize cache-hit in ${Math.round(__t1 - __t0)} ms`); } catch { /* ignore debug logging failure */ }
    ...
    const __logDone = (ok = true) => {
        ...
        } catch { /* ignore debug logging failure */ }
    };
    try {
        requireCppSolidCoreCapability(
        ...
        try {
            ...
            try {
                if (this && Object.prototype.hasOwnProperty.call(this, 'edgeToFillet')) {
                    ...
                    } catch { /* ignore face list extraction failure */ }
                    const failure = {
            ...
            } catch { /* ignore structured diagnostic logging failure */ }
            __logDone(false);
            throw err;
        }
        finally {
...
export function _weldVerticesByEpsilon(eps, options: any = {}) {
    requireCppSolidCoreCapability(
    ...
    try { if (this._manifold && typeof this._manifold.delete === 'function') this._manifold.delete(); } catch { /* ignore stale manifold cleanup failure */ }
    this._manifold = null;
    if (rebuildManifold) {
        this._manifoldize();
    }
