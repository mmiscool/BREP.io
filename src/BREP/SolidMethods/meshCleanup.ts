...
export function removeSmallIslands({ maxTriangles = 30, removeInternal = true, removeExternal = true } = {}) {
    ...
    if (removed > 0) {
        ...
        try { if (this._manifold && typeof this._manifold.delete === "function") this._manifold.delete(); } catch { /* ignore stale manifold cleanup failures */ }
        this._manifold = null;
    }
    return removed;
}
 ...
 */
export function remesh({ maxEdgeLength, maxIterations = 10 }: any = {}) {
...
export function collapseTinyTriangles(lengthThreshold) {
    ...
    try {
        ...
        try { this._faceNameToID = new Map([...this._idToFaceName.entries()].map(([id, name]) => [name, id])); } catch { /* ignore face-name reverse map rebuild failure */ }
        this._dirty = false;
        this._faceIndex = null;
        this._manifold = null; // Rebuild lazily on next need
    } finally {
        try { if (mesh && typeof mesh.delete === 'function') mesh.delete(); } catch { /* ignore mesh disposal failures */ }
    }
...
export function mergeTinyFaces(maxArea = 0.001) {
    ...
    if (merged > 0) {
        ...
        } catch { /* ignore best-effort manifold reindex failure */ }
    }
    return this;
}
