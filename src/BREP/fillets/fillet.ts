...
function solidFromSnapshot(snapshot: any, name: any, opts: any = {}) {
    ...
    try { solid.name = name; } catch { /* ignore readonly solid name failures */ }
    return solid;
}
...
function point3ArrayFromAny(point: any) {
...
function point3ObjectFromAny(point: any) {
    const arr = point3ArrayFromAny(point);
    if (!arr) return null;
    return { x: arr[0], y: arr[1], z: arr[2] };
}

function sanitizeFilletInputPolyline(polylineLocal: any, tolerance = 1e-9) {
 ...
 */
export function attachFilletCenterlineAuxEdge(solid: any, edgeObj: any, radius = 1, sideMode = 'INSET', name = 'FILLET_CENTERLINE', options: any = {}) {
    try {
...
export function filletSolid({
    ...
    // Validate inputs
        ...
        const faceNudgeDistance = Number.isFinite(Number(nudgeFaceDistance))
            ...
            : 0.0001;
        const logDebug = (...args: any[]) => { if (debug) console.log(...args); };
        ...
        if (debug) {
            try { wedgeSolid.visualize(); } catch { /* ignore debug visualization failures */ }
            try { finalSolid.visualize(); } catch { /* ignore debug visualization failures */ }
        }
