...
function combinePathPolylines(edges, tol = 1e-5) {
  ...
  const used: any[] = new Array(polys.length).fill(false);
  const chain: any[] = [];
  ...
  const tryConsumeFromNode = (nodeKey) => {
    ...
    for (const ei of Array.from(node.edges) as any[]) {
      if (used[ei]) continue;
...
function buildSweepEdgeMetadataJson(edge, faceNormal, distance, distanceBack) {
  const metadata: any = { faceType: 'SIDEWALL' };
...
export function generateNativeSweep(target, params: any = {}) {
  ...
  try { target.name = name || 'Sweep'; } catch { /* ignore sweep target naming fallback */ }
  ...
  while (!ok && attempt < 3) {
    try {
      ...
      try { /* probe */ } finally { try { if (mesh && typeof mesh.delete === 'function') mesh.delete(); } catch { /* ignore sweep mesh probe cleanup failure */ } }
      ok = true;
    } catch (error) {
      errLast = error;
      eps *= 2;
      if (!(eps > 0) || eps > 5e-4) break;
      try { target.setEpsilon(eps); } catch { /* ignore sweep epsilon retry failure */ }
    }
    attempt++;
  }
  if (!ok && errLast) {
...
export class Sweep extends Solid {
  ...
  constructor({ face, sweepPathEdges = [], distance = 1, distanceBack = 0, mode = 'translate', name = 'Sweep', omitBaseCap = false, twistAngle = 0 }: any = {}) {
    super();
    this.name = name;
