...
function buildConeHypothesis(activeIndices, activeCount, samples, rng, distEps) {
  ...
  const axisCross = crossXYZ(d01x, d01y, d01z, d02x, d02y, d02z);
  let axis = normalizeXYZ(axisCross[0], axisCross[1], axisCross[2]);
  if (!axis) {
    const normalCross = crossXYZ(n0x, n0y, n0z, n1x, n1y, n1z);
    axis = normalizeXYZ(normalCross[0], normalCross[1], normalCross[2]);
  }
  if (!axis) return null;
  let ux = axis[0];
  let uy = axis[1];
