...
function isSourceSidewallFace(sourceFaceName, metadata: any = {}) {
...
export function groupConnectedFacesBySharedEdges(faces, options: any = {}) {
  ...
  const edgeToFaces = new Map<any, any>();
  ...
  for (let faceIndex = 0; faceIndex < faceList.length; faceIndex++) {
    ...
    const edgeCounts = new Map<any, any>();
    const edgeNormalSums = new Map<any, any>();
    const triCount = index ? ((index.count / 3) | 0) : ((position.count / 3) | 0);
    for (let triIndex = 0; triIndex < triCount; triIndex++) {
      const i0 = index ? (index.getX((triIndex * 3) + 0) >>> 0) : ((triIndex * 3) + 0);
      const i1 = index ? (index.getX((triIndex * 3) + 1) >>> 0) : ((triIndex * 3) + 1);
  ...
  const groupByRoot = new Map<any, any>();
...
function addSmoothAdjacentBoundaryNormals(face, surface, options: any = {}) {
  ...
  const boundaryVertexByPointKey = new Map<any, any>();
  const boundaryEdgeByPointPair = new Map<any, any>();
  ...
  const idToFaceName = solid?._idToFaceName instanceof Map ? solid._idToFaceName : new Map<any, any>();
  ...
  const parentPointCache = new Map<any, any>();
  const parentKeyCache = new Map<any, any>();
  ...
  const equalBoundaryNormalBuckets = useEqualBoundaryNormals ? new Map<any, any>() : null;
...
function analyzeTriangleOrientation(solid) {
  ...
  const edgeUses = new Map<any, any>();
  const addUse = (a, b) => {
...
function orientSolidTrianglesByAdjacency(solid) {
  ...
  const orientationStats = () => {
    const localEdgeUses = new Map<any, any>();
...
function weldSolidVerticesByPosition(solid, epsilon) {
  ...
  oldToNew.fill(-1);
  const keyToNew = new Map<any, any>();
...
function extractFaceSurface(face, options: any = {}) {
  if (!face?.geometry) {
  ...
  const canonicalMap = new Map<any, any>();
  const canonicalAcc = [];
  ...
  const edgeToUses = new Map<any, any>();
  ...
  const boundaryOutgoing = new Map<any, any>();
  ...
  const boundaryEdgeToLoop = new Map<any, any>();
...
function extractFacesSurface(faces, options: any = {}) {
  const faceList = Array.isArray(faces)
    ? faces.filter((face) => face?.geometry)
    : [];
  if (!faceList.length) {
...
function buildThickenClassificationState(labels, distance) {
  ...
  const sidewallGroupByLabel = new Map<any, any>();
  ...
  const faceNameToID = new Map<any, any>();
  const idToFaceName = new Map<any, any>();
...
function buildSolidFromTriangleMesh(mesh, classification, name) {
  ...
  solid._idToFaceName = classification?.idToFaceName instanceof Map
    ...
    : new Map<any, any>();
  solid._faceMetadata = new Map<any, any>();
...
function cullInternalTrianglesByIndexedRaycast(solid, options: any = {}) {
  const vp = Array.isArray(solid?._vertProperties) ? solid._vertProperties : [];
  const tv = Array.isArray(solid?._triVerts) ? solid._triVerts : [];
  const ids = Array.isArray(solid?._triIDs) ? solid._triIDs : [];
  const triCount = (tv.length / 3) | 0;
...
function fillBoundaryLoopsWithTriangles(solid, faceID) {
  ...
  const undirectedAdjacency = new Map<any, any>();
  const directedBoundaryByEdge = new Map<any, any>();
  const boundaryEdges = [];
  ...
  const traceBoundaryGraphLoops = (componentEdges, componentAdjacency) => {
    const vertices: any[] = Array.from(componentAdjacency.keys());
    ...
    const projected = new Map<any, any>();
    ...
    const sortedAdjacency = new Map<any, any>();
    ...
    const simpleCycles = [];
    const verticesSorted = vertices.slice().sort((a: any, b: any) => a - b);
  ...
  for (const edge of boundaryEdges) {
    ...
    const componentAdjacency = new Map<any, any>();
  ...
  for (const fanEdges of edgeFans) {
    const vertices: any[] = Array.from(new Set(fanEdges.flatMap(([a, b]) => [a, b])));
...
function reclassifyThickenCapTrianglesByGeometry(solid, surface, distance, classificationState, tolerance) {
  ...
  const sourceTriRefsByFaceID = new Map<any, any>();
  const offsetTriRefsByFaceID = new Map<any, any>();
...
function repairSolidOrientationByCullAndCap(solid, context: any = {}) {
...
function tryRepairCoherentOrientationCandidate(solid, context: any = {}) {
...
function triangleSplitCullSolid(solid, options: any = {}) {
...
function shouldPreserveExactClosedOffsetShell(staged, surface, distance, options: any = {}, topology = null) {
  const dist = Number(distance);
...
function thickenSurfaceToSolid(surface, face, distance, options: any = {}) {
  const dist = Number(distance);
  ...
  const capLabelsBySourceFaceName = new Map<any, any>();
  ...
  try {
    ...
    if (
      ...
      if (repaired?.solid) {
        ...
        const stats: any = repaired.stats || {};
...
export function thickenFaceToSolid(face, distance, options: any = {}) {
...
export function thickenFacesToSolid(faces, distance, options: any = {}) {
  const faceList = Array.isArray(faces)
    ? faces.filter((face) => face?.geometry)
    : [];
  if (!faceList.length) {
  ...
  return thickenSurfaceToSolid(surface, faceList[0], distance, {
    ...
    sourceFaceMetadataByName: mergeSourceFaceMetadataMaps(
      options.sourceFaceMetadataByName,
      new Map<any, any>(sourceMetadataEntries as any),
    ),
  });
}
