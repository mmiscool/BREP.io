...
type StorageSource = 'local' | 'github' | 'mounted';

type ComponentStorageOptions = {
  source?: string;
  path?: unknown;
  token?: unknown;
  branch?: unknown;
...
type MountedDirectoryHandleLike = {
  kind?: string;
  entries?: () => AsyncIterable<[string, MountedEntryHandleLike]>;
  getDirectoryHandle?: (name: string, options?: { create?: boolean }) => Promise<MountedDirectoryHandleLike>;
  getFileHandle?: (name: string, options?: { create?: boolean }) => Promise<MountedFileHandleLike>;
  removeEntry?: (name: string, options?: { recursive?: boolean }) => Promise<void>;
};

type MountedFileHandleLike = {
  kind?: string;
  getFile?: () => Promise<File>;
  createWritable?: () => Promise<FileSystemWritableFileStream>;
};

type MountedEntryHandleLike = MountedDirectoryHandleLike | MountedFileHandleLike;

type MountedTree = {
  files: Array<{ path: string; handle: MountedFileHandleLike | null }>;
  dirs: string[];
};
...
function resolveComponentPath(name, options: ComponentStorageOptions = {}) {
...
function getGithubPrimaryModelPaths(pathOrName): ModelPathSet {
...
function resolveGithubScope(options: ComponentStorageOptions = {}): GithubScope {
  const cfg = (getGithubStorageConfig() || {}) as ComponentStorageOptions;
...
function getGithubLegacyModelPaths(pathOrName): ModelPathSet {
...
function getGithubRepoTreeCacheKey(scope: GithubScope, repoFull) {
...
async function listGithubTreeRecursive(scope: GithubScope, repoFull, startDir = '') {
  const files = [];
  const dirs = [];
...
async function getGithubRepoTreeSnapshot(scope: GithubScope, repoFull) {
  const cacheKey = getGithubRepoTreeCacheKey(scope, repoFull);
...
async function listGithubComponentRecords(options: ComponentStorageOptions = {}) {
...
async function getGithubComponentRecord(name, options: ComponentStorageOptions = {}) {
  const scope = resolveGithubScope(options);
...
async function setGithubComponentRecord(name, dataObj: ComponentRecordInput, options: ComponentStorageOptions = {}) {
  const scope = resolveGithubScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
...
async function removeGithubComponentRecord(name, options: ComponentStorageOptions = {}) {
  const scope = resolveGithubScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
  if (!scope.token || !repoFull) return;
  const primary = getGithubPrimaryModelPaths(options?.path || name);
...
async function listGithubWorkspaceFolders(options: ComponentStorageOptions = {}) {
  const scope = resolveGithubScope(options);
  const repos = scope.repoFulls;
  if (!scope.token || !repos.length) return [];
  const out = [];
...
async function createGithubWorkspaceFolder(path, options: ComponentStorageOptions = {}) {
  const scope = resolveGithubScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
  if (!scope.token || !repoFull) return;
...
async function removeGithubWorkspaceFolder(path, options: ComponentStorageOptions = {}) {
  const scope = resolveGithubScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
  if (!scope.token || !repoFull) return;
  const folderPath = resolveFolderPath(path);
...
function resolveStorageSource(options: ComponentStorageOptions = {}): StorageSource {
...
function resolveMountedScope(options: ComponentStorageOptions = {}) {
...
function getMountedPrimaryModelPaths(pathOrName): ModelPathSet {
  const modelPath = stripKnownModelExtension(pathOrName);
  if (!modelPath) return { modelPath: '', dataPath: '', metaPath: '', thumbnailPath: '' };
  return {
    modelPath,
...
async function getMountedDirectoryByPath(
  rootHandle: MountedDirectoryHandleLike | null | undefined,
  path,
  { create = false } = {},
) {
...
async function getMountedFileHandleByPath(
  rootHandle: MountedDirectoryHandleLike | null | undefined,
  path,
) {
  const normalized = normalizeComponentPath(path);
...
async function readMountedFileHandleText(fileHandle: MountedFileHandleLike | null | undefined) {
...
async function readMountedFileHandleBase64(fileHandle: MountedFileHandleLike | null | undefined) {
...
async function writeMountedFileBase64(rootHandle: MountedDirectoryHandleLike, path, base64) {
  const normalized = normalizeComponentPath(path);
  if (!normalized) return;
  const { parent, name } = splitParentName(normalized);
  if (!name) return;
...
async function writeMountedFileText(rootHandle: MountedDirectoryHandleLike, path, text) {
  const normalized = normalizeComponentPath(path);
  if (!normalized) return;
  const { parent, name } = splitParentName(normalized);
  if (!name) return;
...
async function removeMountedEntry(rootHandle: MountedDirectoryHandleLike, path, { recursive = false } = {}) {
  const normalized = normalizeComponentPath(path);
  if (!normalized) return;
  const { parent, name } = splitParentName(normalized);
  if (!name) return;
...
async function walkMountedDirectoryTree(
  dirHandle: MountedDirectoryHandleLike | null | undefined,
  basePath,
  out: MountedTree,
) {
...
async function resolveMountedTargets(options: ComponentStorageOptions = {}) {
...
async function listMountedComponentRecords(options: ComponentStorageOptions = {}) {
...
async function getMountedComponentRecord(name, options: ComponentStorageOptions = {}) {
  const scope = resolveMountedScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
...
async function setMountedComponentRecord(name, dataObj: ComponentRecordInput, options: ComponentStorageOptions = {}) {
  const scope = resolveMountedScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
  if (!repoFull) return;
...
async function removeMountedComponentRecord(name, options: ComponentStorageOptions = {}) {
  const scope = resolveMountedScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
  if (!repoFull) return;
  const mount = await getMountedDirectoryHandle(repoFull, { mode: 'readwrite' });
...
async function listMountedWorkspaceFolders(options: ComponentStorageOptions = {}) {
  const targets = await resolveMountedTargets(options);
  const out = [];
...
async function createMountedWorkspaceFolder(path, options: ComponentStorageOptions = {}) {
  const scope = resolveMountedScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
  if (!repoFull) return;
  const mount = await getMountedDirectoryHandle(repoFull, { mode: 'readwrite' });
...
async function removeMountedWorkspaceFolder(path, options: ComponentStorageOptions = {}) {
  const scope = resolveMountedScope(options);
  const repoFull = String(options?.repoFull || scope.repoFull || '').trim();
  if (!repoFull) return;
  const mount = await getMountedDirectoryHandle(repoFull, { mode: 'readwrite' });
...
async function getLocalComponentRecord(name, options: ComponentStorageOptions = {}) {
...
async function setLocalComponentRecord(name, dataObj: ComponentRecordInput, options: ComponentStorageOptions = {}) {
  const modelPath = resolveComponentPath(name, options);
  if (!modelPath) return;
  const key = MODEL_STORAGE_PREFIX + encodeURIComponent(modelPath);
  try {
...
async function removeLocalComponentRecord(name, options: ComponentStorageOptions = {}) {
  const modelPath = resolveComponentPath(name, options);
  if (!modelPath) return;
  const key = MODEL_STORAGE_PREFIX + encodeURIComponent(modelPath);
  try {
...
export async function listComponentRecords(options: ComponentStorageOptions = {}) {
...
export async function listWorkspaceFolders(options: ComponentStorageOptions = {}) {
  const source = resolveStorageSource(options);
  if (source === 'github') {
...
export async function createWorkspaceFolder(path, options: ComponentStorageOptions = {}) {
  const source = resolveStorageSource(options);
  if (source === 'github') {
  ...
  await createLocalWorkspaceFolder(path);
}

export async function removeWorkspaceFolder(path, options: ComponentStorageOptions = {}) {
  const source = resolveStorageSource(options);
  if (source === 'github') {
  ...
  await removeLocalWorkspaceFolder(path);
}

export async function getComponentRecord(name, options: ComponentStorageOptions = {}) {
  const source = resolveStorageSource(options);
  if (source === 'github') {
...
export async function setComponentRecord(name, dataObj: ComponentRecordInput, options: ComponentStorageOptions = {}) {
  const source = resolveStorageSource(options);
  if (source === 'github') {
...
export async function removeComponentRecord(name, options: ComponentStorageOptions = {}) {
  const source = resolveStorageSource(options);
  if (source === 'github') {
