...
type SheetElementRecord = Record<string, any>;
type SheetIdentifier = string | number;
type SheetListener = (sheets: SheetRecord[], partHistory: any | null) => void;
type SheetUpdater = SheetRecord | ((draft: SheetRecord) => SheetRecord | void | null | undefined);
...
export class Sheet2DManager {
  ...
  createSheet(options: SheetRecord = {}) {
  ...
  duplicateSheet(sheetIdOrIndex: SheetIdentifier) {
  ...
  updateSheet(sheetIdOrIndex: SheetIdentifier, updater: SheetUpdater) {
    const index = this._resolveSheetIndex(sheetIdOrIndex);
    if (index < 0) return null;
  ...
  moveSheet(sheetIdOrIndex: SheetIdentifier, toIndex: unknown) {
  ...
  removeSheet(sheetIdOrIndex: SheetIdentifier) {
    const index = this._resolveSheetIndex(sheetIdOrIndex);
    if (index < 0) return null;
  ...
  addListener(listener: SheetListener) {
  ...
  removeListener(listener: SheetListener) {
  ...
  _resolveSheetIndex(sheetIdOrIndex: unknown) {
  ...
  _normalizeSheetsArray(arrayRef: unknown) {
  ...
  _normalizeSheet(rawSheet: unknown, index: number, keepId: unknown = null) {
    const source: SheetRecord = (rawSheet && typeof rawSheet === "object") ? rawSheet as SheetRecord : {};
    ...
    const elements: SheetElementRecord[] = [];
    const ids = new Set();
  ...
  _normalizeElement(rawElement: unknown, index: number, sheetId: string, dims: ResolvedSheetDimensions) {
    const source: SheetElementRecord = (rawElement && typeof rawElement === "object") ? rawElement as SheetElementRecord : {};
  ...
  _emit() {
    ...
    for (const listener of Array.from(this._listeners)) {
      try { listener(sheets, this.partHistory || null); } catch { /* keep other listeners running */ }
    }
  }
}
