...
export class OffsetShellFeature {
  ...
  async run(partHistory) {
    ...
    try { resultSolid.name = newSolidName; } catch { /* ignore result rename failure */ }
    try { resultSolid.visualize(); } catch { /* ignore result visualization failure */ }
    ...
    for (const debugSolid of debugAddedSolids) {
      try { debugSolid.visualize(); } catch { /* ignore debug solid visualization failure */ }
    }
