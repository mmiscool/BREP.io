...
function cloneSolidForRemesh(target: any) {
...
export class RemeshFeature {
  ...
  async run(partHistory: any) {
    ...
    try { outSolid.name = `(${target.name || 'Solid'})`; } catch (_) { /* ignore rename failures */ }
    try { outSolid.visualize(); } catch (_) { /* ignore visualization failures */ }
