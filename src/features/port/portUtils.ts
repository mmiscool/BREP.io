...
export function buildPortDefinitionFromInputs({
  featureId,
  inputParams = {},
...
function createPortVertex(position, name, color) {
  const vertex: any = new BREP.Vertex(position, { name });
  try {
  ...
  } catch { /* ignore material customization failures */ }
  try { vertex.renderOrder = 3; } catch { /* ignore render order failures */ }
  try { if (vertex._point) vertex._point.renderOrder = 3; } catch { /* ignore render order failures */ }
  return vertex;
}
...
export function createPortGroupFromDefinition(definition: any, options: AnyRecord = {}) {
  ...
  try { SelectionFilter.ensureSelectionHandlers(group, { deep: true }); } catch { /* ignore selection handler failures */ }
  return group;
}
