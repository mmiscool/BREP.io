...
function combinePathPolylinesWithUsage(edges, tol = 1e-5) {
  ...
  const tryConsumeFromNode = (nodeKey) => {
    ...
    for (const ei of Array.from(node.edges) as number[]) {
      if (used[ei]) continue;
...
function faceProjectionAverage(solid, faceName, direction) {
  ...
  let sum = 0;
  let count = 0;
  for (const vertexIndex of vertexIndices as Set<number>) {
...
function nudgeTubeEndCaps(tubeSolid, tubeName, distance, { closed = false, debug = false, pathPoints = null }: any = {}) {
  ...
  async run(partHistory) {
    ...
    for (let i = 0; i < tubeTasks.length; i++) {
      ...
      if (inner > 0) {
        ...
        nudgeTubeEndCaps(innerTube, innerName, tubeEndCapNudgeDistance(), {
          closed: isClosedLoop,
          debug: !!debug,
    ...
    const attemptUnionSolids = (solids, label) => {
      ...
      if (solids.length === 1) {
        if (label) {
          try { solids[0].name = label; } catch (_) { /* ignore segment rename failure */ }
        }
        return solids[0];
      }
      try {
        ...
        if (label) {
          try { result.name = label; } catch (_) { /* ignore union rename failure */ }
        }
        return result;
      } catch (error) {
    ...
    if (debug && debugExtras.length) {
      for (const s of debugExtras) {
        try { s.visualize(); } catch (_) { /* ignore debug solid visualization failure */ }
      }
    }
    ...
    for (const solid of baseSolids) {
      if (!solid) continue;
      if (featureID) {
        try { solid.owningFeatureID = featureID; } catch (_) { /* ignore owning feature assignment failure */ }
        if (!solid.name) {
          try { solid.name = featureID; } catch (_) { /* ignore generated solid rename failure */ }
        }
      }
      try { solid.visualize(); } catch (_) { /* ignore generated solid visualization failure */ }
    }
    ...
    if (shouldApplyBoolean) {
      ...
      if (booleanOp === 'UNION' && baseSolids.length > 1) {
        ...
        if (unified) {
          ...
          try { unified.owningFeatureID = featureID; } catch (_) { /* ignore union owning feature assignment failure */ }
          try { unified.visualize(); } catch (_) { /* ignore union visualization failure */ }
        }
      }
