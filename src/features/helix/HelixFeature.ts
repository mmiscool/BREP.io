...
function extractEdgePolylineWorld(edgeObj: any): Vec3Tuple[] {
  const pts: Vec3Tuple[] = [];
  ...
  if (Array.isArray(cached) && cached.length >= 2) {
    if (isWorld) return cached.map((p) => [p[0], p[1], p[2]] as Vec3Tuple);
...
function deriveAxisFromEdge(edgeObj: any) {
...
export class HelixFeature {
  ...
  async run(partHistory: any) { // partHistory reserved for future downstream needs
    ...
    let placementOpts: AnyRecord = { transform: this.inputParams.transform };
    if (placementMode.startsWith("axis")) {
    ...
    const helixData: AnyRecord = BREP.buildHelixPolyline({
      radius,
      endRadius,
      pitch,
      turns,
    ...
    const edge: any = new BREP.Edge(geometry);
    ...
    try {
      if (Array.isArray(helixData.axisLine) && helixData.axisLine.length === 2) {
        ...
        const axisEdge: any = new BREP.Edge(axisGeometry);
        ...
        try {
          const vAxisStart: any = new BREP.Vertex(helixData.axisLine[0] as Vec3Tuple, { name: `${featureId}:AxisStart` });
          vAxisStart.userData = { helixFeatureId: featureId, centerline: true, endpoint: "start" };
          const vAxisEnd: any = new BREP.Vertex(helixData.axisLine[1] as Vec3Tuple, { name: `${featureId}:AxisEnd` });
    ...
    try {
      ...
      const vStart: any = new BREP.Vertex(first as Vec3Tuple, { name: `${featureId}:Start` });
      const vEnd: any = new BREP.Vertex(last as Vec3Tuple, { name: `${featureId}:End` });
