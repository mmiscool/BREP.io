...
function collapseEdgesOnClone(sourceSolid, descriptors, featureID) {
    ...
    try { outSolid.name = sourceSolid.name; } catch { /* ignore clone rename failure */ }
    try { if (featureID) outSolid.owningFeatureID = featureID; } catch { /* ignore owning feature assignment failure */ }
    try { outSolid.visualize(); } catch { /* ignore pre-collapse visualization failure */ }
    ...
    try { outSolid.visualize(); } catch { /* ignore post-collapse visualization failure */ }
...
export class CollapseEdgeFeature {
    ...
    async run() {
        ...
        } catch { /* ignore removal flag assignment failure */ }
