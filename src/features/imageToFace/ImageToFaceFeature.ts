    ...
    actionFunction: (ctx) => {
      ...
      const imageEditor = new ImageEditorUI(fileToImport, {
        onSave: (editedImage) => {
          ...
          try { ctx.feature.inputParams.fileToImport = editedImage; } catch (_) { /* ignore */ }
          try { if (ctx.params) ctx.params.fileToImport = editedImage; } catch (_) { /* ignore */ }
  ...
  async run(partHistory) {
    ...
    const addEdgeSegmentsFromLoop = (loop2D, isHole, manualBreaks, suppressedBreaks) => {
      ...
      for (const seg of segments) {
        ...
        try { lg.computeBoundingSphere(); } catch { /* ignore */ }
    ...
    try { face.edges = edges.slice(); } catch { /* ignore */ }
  ...
  try {
    ...
    if (raw instanceof ArrayBuffer) {
      ...
      try {
        ...
        try { img.close && img.close(); } catch { /* ignore */ }
        return id;
      } catch { /* ignore */ }
      return null;
    }
