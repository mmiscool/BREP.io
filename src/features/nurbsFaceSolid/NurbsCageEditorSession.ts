...
export class NurbsCageEditorSession {
  ...
  constructor(viewer, featureID, options: any = {}) {
  ...
  activate(initialCage, options: any = {}) {
    ...
    this._active = true;
    try { this.viewer.startSplineMode?.(this); } catch { /* ignore best-effort failure */ }
  ...
  dispose() {
    try { this.viewer?.endSplineMode?.(); } catch { /* ignore best-effort failure */ }
  ...
  setCageData(cageData, options: any = {}) {
  ...
  _applyQuadSelectionDelta({ addKeys = [], primaryId = null }: any = {}) {
  ...
  _handleQuadClick({ quadKey, ids, faceDim, faceValue, primaryId = null }: any = {}) {
  ...
  _applySegmentSelectionDelta({ addKeys = [], removeKeys = [], primaryId = null }: any = {}) {
    const add = Array.from(new Set((Array.isArray(addKeys) ? addKeys : []).map((k) => String(k || "")).filter(Boolean)));
  ...
  _ensurePointMaterials() {
    ...
    try { this._pointMaterials.base.color.setHex(this._displayOptions.cageColor); } catch { /* ignore best-effort failure */ }
  }

  _disposePointMaterials() {
    const mats = this._pointMaterials;
    if (!mats) return;
    try { mats.base?.dispose?.(); } catch { /* ignore best-effort failure */ }
    try { mats.selected?.dispose?.(); } catch { /* ignore best-effort failure */ }
    this._pointMaterials = null;
  }
  ...
  _disposePointGeometry() {
    try { this._pointGeometry?.dispose?.(); } catch { /* ignore best-effort failure */ }
    this._pointGeometry = null;
    this._pointGeometryRadius = 0;
  }
  ...
  _buildPreviewGroup() {
    ...
    if (existing) {
      try { scene.remove(existing); } catch { /* ignore best-effort failure */ }
    }
  ...
  _destroyPreviewGroup() {
    ...
    try {
      while (this._previewGroup.children.length) {
        ...
        try { child.geometry?.dispose?.(); } catch { /* ignore best-effort failure */ }
        try { child.material?.dispose?.(); } catch { /* ignore best-effort failure */ }
        try { child.userData?.__hoverMaterial?.dispose?.(); } catch { /* ignore best-effort failure */ }
      }
    } catch { /* ignore best-effort failure */ }
    try {
  ...
  _createControl() {
    ...
    markSceneOverlayObject(control as any, {
      preserve: true,
  ...
  _destroyControl() {
    ...
    if (listeners) {
      try { this._control.removeEventListener("change", listeners.onChange); } catch { /* ignore best-effort failure */ }
      try { this._control.removeEventListener("dragging-changed", listeners.onDragging); } catch { /* ignore best-effort failure */ }
    }
    try { this._control.detach?.(); } catch { /* ignore best-effort failure */ }
    try { allowSceneOverlayRemoval(this._control, { deep: true }); } catch { /* ignore best-effort failure */ }
    try { this.viewer?.scene?.remove(this._control); } catch { /* ignore best-effort failure */ }
    try { this._control.dispose?.(); } catch { /* ignore best-effort failure */ }
    this._control = null;
    this._controlListeners = null;
  }
  ...
  _setupViewerControlsListener() {
    ...
    this._controlsListener = () => {
      try { this._control?.update?.(); } catch { /* ignore best-effort failure */ }
      this._updatePointHandleScales();
      this._updateQuadPickGeometry();
    };
    try { this.viewer.controls.addEventListener("change", this._controlsListener); } catch { /* ignore best-effort failure */ }
    try { this.viewer.controls.addEventListener("end", this._controlsListener); } catch { /* ignore best-effort failure */ }
  }

  _teardownViewerControlsListener() {
    if (!this._controlsListener || !this.viewer?.controls) return;
    try { this.viewer.controls.removeEventListener("change", this._controlsListener); } catch { /* ignore best-effort failure */ }
    try { this.viewer.controls.removeEventListener("end", this._controlsListener); } catch { /* ignore best-effort failure */ }
    this._controlsListener = null;
  }
  ...
  _applySymmetryFromSources(sourceIdsInput, options: any = {}) {
  ...
  _applyDisplayOptions() {
    ...
    if (this._lineObject) {
      ...
      try { this._lineObject.material?.color?.setHex?.(options.cageColor); } catch { /* ignore best-effort failure */ }
    }
    ...
    for (const quad of this._quadPickObjects) {
      ...
      if (baseMat) {
        try { baseMat.color?.setHex?.(options.cageColor); } catch { /* ignore best-effort failure */ }
        try { baseMat.opacity = CAGE_QUAD_IDLE_OPACITY; } catch { /* ignore best-effort failure */ }
      ...
      if (hoverMat) {
        try { hoverMat.color?.setHex?.(options.cageColor); } catch { /* ignore best-effort failure */ }
        try { hoverMat.opacity = CAGE_QUAD_FILL_OPACITY; } catch { /* ignore best-effort failure */ }
      }
    }
  ...
  _handleControlChange() {
    ...
    const symmetryApplyOptions: any = {
      dims,
    };
    if (symmetryCenter) {
  ...
  _renderOnce() {
    try { this.viewer?.render?.(); } catch { /* ignore best-effort failure */ }
  }
