...
function updateHashText32(hash, value: any = '') {
    const text = String(value == null ? '' : value);
    let next = hash >>> 0;
    for (let i = 0; i < text.length; i += 1) {
...
function updateHashArrayLike32(hash, label, values, numeric = false) {
    const arr: any = isHashableArrayLike(values) ? values : [];
...
function updateHashMap32(hash, label, map, valueHash: any = (value) => String(value)) {
...
function cloneAuthoringSnapshot(snapshot: any) {
    ...
    return {
        ...
        auxEdges: Array.from(snapshot.auxEdges ?? [], (entry: any) => ({
            ...entry,
            points: Array.from(entry?.points ?? [], (point: any) => Array.from(point ?? [])),
        })),
    };
}
...
export class FilletFeature {
    ...
    async run(partHistory: any) {
        ...
        let { solid: targetSolid, solids }: any = resolveSingleSolidFromEdges(edgeObjs);
        ...
        if (!targetSolid) {
            if (solids.size > 1) {
                console.warn('[FilletFeature] Edges reference multiple solids; aborting fillet.', { solids: Array.from(solids).map((s: any) => s?.name) });
            } else {
        ...
        } catch { /* ignore fillet metadata marker failure */ }
        const collectDebugSolids = (res) => {
            ...
            for (const dbg of res.__debugAddedSolids) {
                ...
                } catch { /* ignore debug solid rename failure */ }
