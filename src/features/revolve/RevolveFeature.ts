...
export class RevolveFeature {
    ...
    static showContexButton(selectedItems: any) {
        ...
        const params: AnyRecord = { profile: profileName };
    ...
    _resolveReference(entry: any, partHistory: any) {
        const resolved = resolveSelectionObject(entry, partHistory);
        if (resolved) return resolved;

        const refName = normalizeSelectionName(entry);
        if (!refName) return null;
        try { return partHistory?.getObjectByName?.(refName) || null; } catch { /* ignore lookup failures */ }
        try { return partHistory?.scene?.getObjectByName?.(refName) || null; } catch { /* ignore lookup failures */ }
        return null;
    }
    ...
    async run(partHistory: any) {
        ...
        try { revolve.setEpsilon(1e-6); } catch { /* ignore epsilon failures */ }
        revolve.visualize();
        ...
        try { for (const obj of removedArtifacts) { if (obj) obj.__removeFlag = true; } } catch { /* ignore remove flag failures */ }
        return {
