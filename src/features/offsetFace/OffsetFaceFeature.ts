...
const getGeometryCenter = (geom) => {
  ...
  try {
    ...
    // Bounding boxes are best-effort; fall back to averaging positions below.
  }
...
const getEdgePolylineWorld = (edgeObj) => {
  ...
  try { edgeObj.updateMatrixWorld(true); } catch {
    // Continue with the current matrix when a legacy object cannot update.
  }
...
const buildEdgesFromSourceFace = (faceObj, normal, groupName, offsetVec, edgeBias, applySketchEdgeStyle) => {
  ...
  for (let edgeIdx = 0; edgeIdx < descriptors.length; edgeIdx++) {
    ...
    try { lg.computeBoundingSphere(); } catch {
      // Line bounding spheres are best-effort for selection/rendering.
    }
...
const addVerticesFromEdges = (group, edges, featureId, groupName) => {
  ...
  const addPoint = (point) => {
    ...
    vertexIdx += 1;
    try {
      const vertex = new BREP.Vertex(p as [number, number, number], { name: vertexName });
      ...
      // Vertices are optional helpers; keep the offset face usable without them.
    }
  };
  ...
  async run(partHistory: any) {
    ...
    for (const entry of faceEntries) {
      ...
      try {
        ...
        // Material styling is best-effort; geometry remains valid without it.
      }
      ...
      try { offsetFace.updateMatrixWorld(true); } catch {
        // Boundary recovery can use the existing matrix.
      }
