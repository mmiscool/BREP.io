...
function buildSphereSource(feature) {
  ...
  try {
    ...
    try { mesh?.delete?.(); } catch { /* ignore best-effort failure */ }
    try { sphere?.free?.(); } catch { /* ignore best-effort failure */ }
    try { sphere?.delete?.(); } catch { /* ignore best-effort failure */ }
  }
}
...
function buildTorusSource(feature) {
  ...
  try {
    ...
    try { mesh?.delete?.(); } catch { /* ignore best-effort failure */ }
    try { torus?.free?.(); } catch { /* ignore best-effort failure */ }
    try { torus?.delete?.(); } catch { /* ignore best-effort failure */ }
  }
}
...
function loopSubdivideOnce(meshDataInput) {
  ...
  const nextVertices = oldVertices.map((v, vertexIndex) => {
    ...
    const boundary = Array.from(boundaryNeighbors[vertexIndex]) as any[];
    ...
    const neighbors = Array.from(vertexNeighbors[vertexIndex]) as any[];
  ...
  for (const child of solid.children) {
    ...
    if (child.type === "FACE" && child.material) {
      ...
      if (typeof faceMat.clone === "function") {
        try { faceMat = faceMat.clone(); } catch { /* ignore best-effort failure */ }
      }
      try { faceMat?.color?.setHex?.(faceColor); } catch { /* ignore best-effort failure */ }
      try { faceMat.transparent = true; } catch { /* ignore best-effort failure */ }
      try { faceMat.opacity = 0.92; } catch { /* ignore best-effort failure */ }
      try { faceMat.emissive?.setHex?.(faceColor); } catch { /* ignore best-effort failure */ }
      try { faceMat.emissiveIntensity = 0.08; } catch { /* ignore best-effort failure */ }
    ...
    if (child.type === "EDGE") {
      ...
      if (edgeMat && typeof edgeMat.clone === "function") {
        try { edgeMat = edgeMat.clone(); } catch { /* ignore best-effort failure */ }
      }
      try { edgeMat?.color?.setHex?.(edgeColor); } catch { /* ignore best-effort failure */ }
...
function renderPolygonEditorWidget({ ui, key, controlWrap, row }) {
  ...
  const handleSessionSelectionChange = (id, details: any = {}) => {
  ...
  const disposeSession = () => {
    if (!state.session) return;
    try { state.session.dispose(); } catch { /* ignore best-effort failure */ }
    state.session = null;
  };
