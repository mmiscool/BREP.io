...
export class PolygonSolidEditorSession {
  ...
  activate(initialMeshData, options: any = {}) {
  ...
  setMeshData(meshData, options: any = {}) {
  ...
  _ensurePointMaterials() {
    ...
    try { this._pointMaterials.base.color.setHex(this._displayOptions.meshColor); } catch { /* ignore best-effort failure */ }
  }
  ...
  _destroyPreviewGroup() {
    ...
    try {
      while (this._previewGroup.children.length) {
        ...
        try { child.userData?.__selectedMaterial?.dispose?.(); } catch { /* ignore best-effort failure */ }
      }
  ...
  _applyDisplayOptions() {
    ...
    if (this._lineObject) {
      ...
      try { this._lineObject.material?.color?.setHex?.(options.meshColor); } catch { /* ignore best-effort failure */ }
    }
    ...
    for (const edgePick of this._edgePickObjectsByKey.values()) {
      ...
      if (idleMat) {
        try { idleMat.color?.setHex?.(options.meshColor); } catch { /* ignore best-effort failure */ }
        edgePick.userData.__idleMaterial = idleMat;
      }
      if (hoverMat) {
        try { hoverMat.color?.setHex?.(options.meshColor); } catch { /* ignore best-effort failure */ }
      }
    }
    ...
    for (const triPick of this._trianglePickObjectsByKey.values()) {
      ...
      if (idleMat) {
        try { idleMat.color?.setHex?.(options.meshColor); } catch { /* ignore best-effort failure */ }
        try { idleMat.opacity = TRI_IDLE_OPACITY; } catch { /* ignore best-effort failure */ }
        triPick.userData.__idleMaterial = idleMat;
      }
      if (hoverMat) {
        try { hoverMat.color?.setHex?.(options.meshColor); } catch { /* ignore best-effort failure */ }
        try { hoverMat.opacity = TRI_HOVER_OPACITY; } catch { /* ignore best-effort failure */ }
      }
    }
  ...
  _notifyMeshChange(reason, details: any = {}) {
    try {
