...
export class BooleanFeature {
    ...
    async run(partHistory: any) {
        ...
        let effects: AnyRecord = { added: [], removed: [] };
        if (op === 'SUBTRACT') {
            ...
            const param = {
                operation: 'SUBTRACT',
                targets: [target],
                overlapConditioningEnabled: bool.overlapConditioningEnabled !== false,
            };
        ...
        try { for (const obj of effects.removed || []) { if (obj) obj.__removeFlag = true; } } catch { /* ignore remove flag failures */ }
        return {
