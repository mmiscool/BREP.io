...
function resolveTargetSolid(targetSolid: any, partHistory: any) {
...
function cloneSolidForCleanup(target: any) {
  ...
  const fallback: any = new BREP.Solid();
...
function isSingleSolidSelection(selectedItems: any) {
...
function countTouchedTriangles(intersections: any) {
...
export class SelfIntersectionCleanupFeature {
  ...
  async run(partHistory: any) {
    ...
    try { outSolid.name = `${baseName}_SelfIntersectionCleanup`; } catch { /* ignore rename failures */ }
    try { outSolid.visualize(); } catch { /* ignore visualization failures */ }

    try { target.__removeFlag = true; } catch { /* ignore remove flag failures */ }
