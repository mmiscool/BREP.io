...
function weldSolidVerticesByQuantizedPosition(solid, quantum = COORD_QUANT) {
  ...
  try { if (solid._manifold && typeof solid._manifold.delete === "function") solid._manifold.delete(); } catch { /* ignore manifold cleanup failure */ }
...
function parseDashPattern(style: any = {}) {
  ...
  for (let i = 0; i < entities.length; i += 1) {
    ...
    if (entity.type === "line" || entity.type === "polyline") {
      ...
      if (line.material?.isLineDashedMaterial) {
        try { line.computeLineDistances(); } catch { /* ignore dashed-line distance failure */ }
      }
