...
type FlatPatternFileOptions = {
  baseName?: any;
  directory?: any;
  fallbackBaseName?: any;
  format?: any;
...
function sanitizeFlatPatternFileStem(raw: any, fallback = "flat_pattern") {
...
function modelPathToFlatPatternBaseName(raw: any, fallback = "part") {
...
export function resolveViewerFlatPatternBaseName(viewer: any, fallback = "part") {
  const candidates = [
...
function isSheetMetalSolid(object: any): object is AnyRecord {
  return !!(
    object
    && object.type === "SOLID"
...
function collectSheetMetalSolidsFromScene(scene: any, options: FlatPatternFileOptions = {}) {
...
export function collectSheetMetalSolidsFromViewer(viewer: any, options: FlatPatternFileOptions = {}) {
  const scene = viewer?.partHistory?.scene || viewer?.scene;
  return collectSheetMetalSolidsFromScene(scene, options);
}

function resolveSheetMetalFeatureName(solid: any, index = 0) {
  const candidates = [
...
function uniqueStem(baseStem: any, usedStems: Map<string, number>) {
...
function normalizeDirectory(raw: any) {
  return String(raw ?? "")
    .replace(/\\/g, "/")
    .split("/")
...
export function buildSheetMetalFlatPatternFiles(solids: any, options: FlatPatternFileOptions = {}) {
...
export function buildSheetMetalFlatPatternPackageFiles(solids: any, options: FlatPatternFileOptions = {}) {
