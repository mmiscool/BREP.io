...
function handleComponentSelection(ctx: any, record: any) {
...
export class AssemblyComponentFeature {
  ...
  static longName = 'ASSY Component';
  ...
  async run(partHistory: any) {
    try { console.log('[AssemblyComponentFeature] run: begin', { componentName: this.inputParams?.componentName || null, featureID: this.inputParams?.featureID || null }); } catch { /* ignore logging failure */ }
    ...
    if (!componentData || !componentData.bytes || componentData.bytes.length === 0) {
      ...
      try { console.warn('[AssemblyComponentFeature] run: abort (no component data)'); } catch { /* ignore logging failure */ }
    ...
    } catch { /* ignore debug stats failure */ }
    ...
    if (!solids.length) {
      ...
      try { console.warn('[AssemblyComponentFeature] run: no solids recovered'); } catch { /* ignore logging failure */ }
      return { added: [], removed: [] };
    }
    ...
    const component: any = new BREP.AssemblyComponent({
      name: componentName,
    ...
    try { console.log('[AssemblyComponentFeature] run: complete', { componentName, solids: solids.length }); } catch { /* ignore logging failure */ }
  ...
  async _resolveComponentData() {
    ...
    const record: any = await getComponentRecord(selectedPath, {
  ...
  async _buildSolidsFromGroup(group, componentData) {
    ...
    if (!solids.length && meshes.length) {
      try {
        ...
        for (const mesh of meshes) {
          ...
          catch { /* ignore geometry transform failure */ }
        ...
        if (worldGeometries.length) {
          ...
          if (merged) {
            try {
              ...
              try { merged.dispose(); } catch { /* ignore geometry disposal failure */ }
            }
          }
        }
        for (const g of worldGeometries) {
          try { g.dispose(); } catch { /* ignore geometry disposal failure */ }
        }
      } catch (err) {
    ...
    try { console.log('[AssemblyComponentFeature] _buildSolidsFromGroup: complete', { componentName, solids: solids.length }); } catch { /* ignore logging failure */ }
    return solids;
  }
  ...
  _applyBrepExtrasToSolid(solid, extras) {
    ...
    if (extras.edgeMetadata && typeof extras.edgeMetadata === 'object') {
      ...
      for (const [edgeName, meta] of Object.entries(extras.edgeMetadata)) {
        ...
        const existing = merged.get(edgeName);
        merged.set(edgeName, {
  ...
  _appendMeshToSolid(solid, mesh, faceNameCounts, options: any = {}) {
  ...
  async _rebuildSolidsFromHistory(componentData, componentName) {
    try {
      ...
      for (const child of children) {
        ...
        try { sandbox.scene.remove(child); } catch { /* ignore scene removal failure */ }
        child.parent = null;
        ...
        if (child.type !== 'COMPONENT') {
          ...
          if (faceMeta && typeof child.setFaceMetadata === 'function') {
            for (const key of Object.keys(faceMeta)) {
              try { child.setFaceMetadata(key, faceMeta[key]); } catch { /* ignore metadata write failure */ }
            }
          }
  ...
  _toArrayBuffer(uint8): ArrayBuffer | null {
    if (!(uint8 instanceof Uint8Array)) return null;
    if (uint8.byteOffset === 0 && uint8.byteLength === uint8.buffer.byteLength) {
      return uint8.buffer instanceof ArrayBuffer ? uint8.buffer : uint8.slice().buffer;
    }
