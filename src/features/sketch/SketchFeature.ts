...
function renderEditSketchButtonField(args: any = {}) {
...
export class SketchFeature {
    ...
    constructor(_partHistory: any = null) {
        this.inputParams = {};
    ...
    dumpDiagnostics({ partHistory, download = false, fileName }: AnyRecord = {}) {
        try {
    ...
    async run(partHistory: any) {
        ...
        const applySketchEdgeStyle = (edge) => {
            if (!edge) return;
            edge.renderOrder = 2;
            try { edge.computeLineDistances?.(); } catch { /* ignore */ }
            try {
        ...
        try {
            ...
            if (scene && refs.length) {
                ...
                const ptById = new Map<any, any>(sketch.points.map((p) => [p.id, p]));
                let changed = false;
                for (const r of refs) {
                    try {
                        ...
                        if (!edge || edge.type !== 'EDGE') {
                            ...
                            if (edge && edge.type === 'EDGE') {
                                ...
                                r.edgeId = edge.id;
                                try { r.edgeName = edge.name || r.edgeName || null; } catch { /* ignore */ }
                                try { r.solidName = edge.parent?.name || r.solidName || null; } catch { /* ignore */ }
                                changed = true;
                            }
                        }
        ...
        const pointById = new Map<any, any>(sketch.points.map((p) => [p.id, { x: p.x, y: p.y }]));
        const segs = [];
        const edges = [];
        const openChains = [];
        ...
        if (groups.length){
            ...
            for (const grp of groups){
                ...
                for (const sid of (loopSegIDs[grp.outer] || [])) {
                    ...
                    if (e) {
                        try { e.userData = e.userData || {}; e.userData.isHole = false; } catch { /* ignore */ }
                        boundaryEdges.add(e);
                    }
                }
                const holes = grp.holes.map(idx=>{
                    ...
                    for (const sid of (loopSegIDs[idx] || [])) {
                        const e = edgeBySegId.get(segs[sid]?.id);
                        if (e) {
                            try { e.userData = e.userData || {}; e.userData.isHole = true; } catch { /* ignore */ }
                            boundaryEdges.add(e);
                        }
                    }
                    return h;
            ...
            if (triPositions.length){
                ...
                this.persistentData.lastProfileDiagnostics = {
                    ...
                    boundaryEdges: Array.from(boundaryEdges as Iterable<any>).map((edge) => edge?.name || null),
                    edges: diagEdges,
