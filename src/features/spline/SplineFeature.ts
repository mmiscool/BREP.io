...
function renderSplinePointsWidget({ ui, key, controlWrap, row }) {
  ...
  const attachPointToPortOwner = (pointId, portOwner, options: AnyRecord = {}) => {
  ...
  const disposeSession = (_force = false) => {
  ...
  const commit = (reason, options: AnyRecord = {}) => {
  ...
  async run(partHistory) {
    ...
    if (positions.length >= 6) {
      ...
      const edgeMaterial = CADmaterials?.EDGE?.OVERLAY?.clone?.()
        ...
        || edge.material?.clone?.()
        || edge.material;
      try { edgeMaterial.color?.set?.(0xffffff); } catch { /* ignore material color assignment failures */ }
      try { edgeMaterial.depthTest = false; } catch { /* ignore material depth-test assignment failures */ }
      try { edgeMaterial.depthWrite = false; } catch { /* ignore material depth-write assignment failures */ }
      try { edgeMaterial.transparent = true; } catch { /* ignore material transparency assignment failures */ }
    ...
    try {
      const vertices = evaluatedSpline.points.map((pt, idx) => {
        ...
        const pointMaterial = vertex?._point?.material?.clone?.()
          ...
          || null;
        if (pointMaterial) {
          try { pointMaterial.depthTest = false; } catch { /* ignore material depth-test assignment failures */ }
          try { pointMaterial.depthWrite = false; } catch { /* ignore material depth-write assignment failures */ }
          SelectionState.setBaseMaterial(vertex, pointMaterial, { force: true });
        }
        try { vertex.renderOrder = 10001; } catch { /* ignore render-order assignment failures */ }
        try { if (vertex._point) vertex._point.renderOrder = 10001; } catch { /* ignore point render-order assignment failures */ }
