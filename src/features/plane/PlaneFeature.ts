...
export class PlaneFeature {
    ...
    #basisFromOrientation(orientation: any) {
    ...
    #basisFromReference(refObj: any) {
        if (!refObj) return null;
        const origin = new THREE.Vector3();
        try { refObj.updateWorldMatrix(true, true); } catch { /* ignore matrix update failures */ }
        ...
        if (refObj.type === 'FACE') {
            ...
            if (!n || n.lengthSq() < 1e-12) {
                ...
                try { refObj.getWorldQuaternion(q); } catch { /* ignore quaternion lookup failures */ }
