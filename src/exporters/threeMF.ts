...
type ThreeMfOptions = {
  ...
  precision?: number;
  scale?: number;
  metadataManager?: MetadataManagerLike | null;
  useMetadataColors?: boolean;
  includeFaceTags?: boolean;
  applyWorldTransform?: boolean;
  defaultFaceColor?: unknown;
  modelMetadata?: Record<string, unknown> | null;
  thumbnail?: string | Uint8Array | null;
  additionalFiles?: Record<string, unknown> | null;
};

type RootRelsOptions = {
 ...
 */
export function computeTriangleMaterialIndices(solid, mesh, opts: ThreeMfOptions = {}) {
  try {
 ...
 * @returns {string}
 */
function build3MFModelXML(solids, opts: ThreeMfOptions = {}) {
  ...
  for (const s of (solids || [])) {
      ...
      if (uniqueIds.length > 0) {
        ...
        for (let i = 0; i < uniqueIds.length; i++) {
          ...
          try {
            ...
            // Keep the neutral fallback color if hashing fails.
          }
    ...
    solidIdx++;
      ...
      try { if (mesh && typeof mesh.delete === 'function') mesh.delete(); } catch {
        // Ignore mesh disposal failures during export cleanup.
      }
    }
  }
...
function rootRelsXML({ thumbnailPath, viewImages, attachments }: RootRelsOptions = {}) {
  const lines = [];
  lines.push('<?xml version="1.0" encoding="UTF-8"?>');
 ...
 */
export async function generate3MF(solids, opts: ThreeMfOptions = {}) {
