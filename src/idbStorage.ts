...
function getSettingsStorage() {
  ...
  } catch { /* ignore blocked browser settings storage access */ }
  return null;
}

function readLegacyKey(key: any) {
  try {
    if (typeof window !== 'undefined' && window.sessionStorage) {
      const v = window.sessionStorage.getItem(key);
      if (v) return v;
    }
  } catch { /* ignore blocked sessionStorage access */ }
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
      return window.localStorage.getItem(key) || '';
    }
  } catch { /* ignore blocked localStorage access */ }
  return '';
}

function normalizeStorageMode(mode: any) {
...
function normalizeRepoFullList(input: any) {
...
function clearLegacySettings() {
  ...
  } catch { /* ignore legacy sessionStorage cleanup failures */ }
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
  ...
  } catch { /* ignore legacy localStorage cleanup failures */ }
}

function saveStorageSettings(settings: any, { clearLegacy = false }: any = {}) {
...
function saveStorageMode(mode: any) {
...
function saveGithubConfig({ token, repoFull, repoFulls, branch }: any) {
  const settings = loadStorageSettings();
...
function tryDispatchStorageEvent(storage: any, { key, oldValue, newValue }: any) {
  try {
    if (typeof window !== 'undefined') {
      let ev;
      try {
  ...
  } catch { /* ignore storage event dispatch failures */ }
}

function joinPath(...parts: any[]) {
  return parts.filter(Boolean).join('/');
}

function encodeSettingKey(key: any) {
  const k = String(key ?? '');
...
function decodeSettingKey(name: any) {
  if (/%[0-9A-Fa-f]{2}/.test(name)) {
    try { return decodeURIComponent(name); } catch { /* fall through to raw setting key */ }
  }
  return name;
}

function isModelKey(key: any) {
  return typeof key === 'string' && key.startsWith(MODEL_PREFIX);
}

function isLegacyModelKey(key: any) {
  return typeof key === 'string' && key.startsWith(LEGACY_MODEL_PREFIX);
}

function normalizeModelKey(key: any) {
  if (isModelKey(key)) return key;
  if (isLegacyModelKey(key)) return MODEL_PREFIX + key.slice(LEGACY_MODEL_PREFIX.length);
  return null;
}

function keyToPath(key: any) {
  if (isModelKey(key)) {
...
async function ensureDir(path: any) {
  try {
    await fsAny.promises.mkdir(path, { recursive: true });
  } catch (e) {
...
async function readFileSafe(path: any) {
  try {
    return await fsAny.promises.readFile(path, 'utf8');
  } catch {
    return null;
  }
}

async function readDirSafe(path: any) {
  try {
    return await fsAny.promises.readdir(path);
  } catch {
    return [];
  }
}
...
async function openLegacyDb() {
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    req.onsuccess = () => {
      const db = req.result;
      if (created) {
        try { db.close(); } catch { /* ignore close failure for newly created legacy DB */ }
...
async function readAllLegacyEntries(db: any) {
  return new Promise<any>((resolve, reject) => {
    try {
...
class VfsStorage {
  ...
  async _writeKeyToFs(key, value) {
    ...
    await fsAny.promises.writeFile(path, toStringValue(value), 'utf8');
  }

  async _removeKeyFromFs(key) {
    const path = keyToPath(key);
    try {
      await fsAny.promises.unlink(path);
    } catch (e) {
  ...
  async _resetDir(path) {
    try {
      await fsAny.promises.rm(path, { recursive: true, force: true });
    } catch { /* ignore missing or locked storage directories */ }
  ...
  async _migrateLegacyIdb() {
    ...
    try {
      ...
      try { db && db.close && db.close(); } catch { /* ignore legacy DB close failures */ }
    }
  ...
  async _migrateLegacyFs() {
    ...
    for (const name of settingFiles) {
      ...
      try { await fsAny.promises.unlink(legacyPath); } catch { /* ignore legacy file cleanup failures */ }
    }
    ...
    for (const name of legacyFiles) {
      ...
      if (await exists(to)) {
        try { await fsAny.promises.unlink(from); } catch { /* ignore duplicate legacy file cleanup failures */ }
        continue;
      }
      try {
        await fsAny.promises.rename(from, to);
      } catch {
        const raw = await readFileSafe(from);
        if (raw !== null) {
          await fsAny.promises.writeFile(to, raw, 'utf8');
        }
        try { await fsAny.promises.unlink(from); } catch { /* ignore copied legacy file cleanup failures */ }
      }
    }
    try { await fsAny.promises.rm(legacyDir, { recursive: true, force: true }); } catch { /* ignore legacy directory cleanup failures */ }
  }
  ...
  _setupBroadcast() {
    ...
    } catch { /* ignore BroadcastChannel setup failures */ }
  }
  ...
  setItem(key: any, value: any) {
    ...
    try { this._bc?.postMessage({ type: 'set', key: k, newValue: v, oldValue }); } catch { /* ignore broadcast failures */ }
  }
  ...
  removeItem(key: any) {
    ...
    try { this._bc?.postMessage({ type: 'remove', key: k, oldValue, newValue: null }); } catch { /* ignore broadcast failures */ }
  }
  ...
  clear() {
    ...
    tryDispatchStorageEvent(this, { key: null, oldValue: null, newValue: null });
    try { this._bc?.postMessage({ type: 'clear' }); } catch { /* ignore broadcast failures */ }
  }
...
class StorageProxy {
  ...
  constructor(localBackend: any, githubBackend: any) {
  ...
  _dispatchBackendEvent() {
    ...
    } catch { /* ignore backend change event dispatch failures */ }
  }

  async useGithub(config: any) {
...
async function applyStorageMode(mode: any, config: any) {
...
async function setStorageMode(mode: any, { persist = true }: any = {}) {
  if (persist) saveStorageMode(mode);
  const cfg = loadGithubConfig();
  return await applyStorageMode(mode, cfg);
}

async function configureGithubStorage({ token, repoFull, repoFulls, branch, persist = true, mode }: any = {}) {
