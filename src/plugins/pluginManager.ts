...
async function fetchAndPrepareEntryViaWorker(entryUrls, baseUrls, ts) {
  return new Promise<WorkerPreparedEntry>((resolve, reject) => {
    const worker = new Worker(new URL('./ghLoader.worker.ts', import.meta.url), { type: 'module' });
    const cleanup = () => {
      try { worker.terminate(); } catch {
        // Ignore worker termination failures during plugin cleanup.
      }
    };
    worker.onmessage = (ev) => {
      const data = ev.data || {};
...
export async function importGithubPlugin(repoUrl) {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      try { (__cleanup || (()=>{}))(); } catch {
        // Ignore worker dependency cleanup failures.
      }
  ...
  const app = {
    ...
    registerFeature(FeatureClass, options = null) {
      try {
        ...
        // Ignore plugin feature registration failures.
      }
    },
    registerAnnotation(AnnotationHandler) {
      try {
        ...
        // Ignore plugin annotation registration failures.
      }
    },
    addToolbarButton(labelOrSpec, title, onClick) {
      ...
      try {
        ...
        // Ignore plugin toolbar registration failures.
      }
    },
...
export function savePluginUrls(urls) {
  try {
    writeBrowserStorageValue(STORAGE_KEY, JSON.stringify((urls || []).map(s => String(s || '').trim()).filter(Boolean)));
  } catch {
    // Ignore plugin URL persistence failures.
  }
}
...
export function savePluginEnabledMap(map) {
  try {
    ...
    // Ignore plugin enabled-state persistence failures.
  }
}
