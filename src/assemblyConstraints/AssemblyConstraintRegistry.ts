...
export type AssemblyConstraintClassLike = {
  new (...args: any[]): any;
  name?: string;
  shortName?: string;
  longName?: string;
  constraintShortName?: string;
  constraintName?: string;
  constraintType?: string;
  type?: string;
  aliases?: string[];
  inputParamsSchema?: unknown;
  showContexButton?: (...args: any[]) => unknown;
};

export class AssemblyConstraintRegistry {
  _map: Map<string, AssemblyConstraintClassLike>;
  _aliases: Map<string, AssemblyConstraintClassLike>;
   ...
   */
  register(ConstraintClass: AssemblyConstraintClassLike | null | undefined) {
  ...
  get(name: unknown): AssemblyConstraintClassLike {
  ...
  getSafe(name: unknown): AssemblyConstraintClassLike | null {
    try {
  ...
  list(): AssemblyConstraintClassLike[] {
    return Array.from(new Set(this._map.values()));
  }

  listAvailable(): AssemblyConstraintClassLike[] {
    return this.list();
  }

  #collectKeys(ConstraintClass: AssemblyConstraintClassLike): { typeKey: string | null; aliases: string[] } {
    ...
    const aliases = Array.isArray(ConstraintClass.aliases)
      ? ConstraintClass.aliases.filter(Boolean).map(String)
      : [];
