...
type ConstraintEntry = AnyRecord;
type ConstraintListener = (payload: AnyRecord) => void;
...
function extractDefaults(schema: any): AnyRecord {
...
function formatUnknownConstraintMessage(type: any) {
...
function clampIterations(value: any) {
...
function clampGain(value: any, fallback: number) {
...
function removeExistingDebugArrows(scene: any) {
  ...
  for (const obj of toRemove) {
    ...
    catch { /* ignore debug cleanup failures */ }
  }
}

function resolveComponentFromObject(obj: any) {
  let current = obj;
  while (current) {
...
function scoreObjectForComponent(object: any) {
  return resolveComponentFromObject(object) ? 1 : 0;
}

function vectorFrom(value: any): THREE.Vector3 | null {
...
function resolveConstraintEntryId(entry: any, fallback: any = null): string | null {
...
function normalizeConstraintEntryId(entry: any) {
...
export class AssemblyConstraintHistory {
  ...
  idCounter: number;
  _listeners: Set<ConstraintListener>;
  _autoRunScheduled: boolean;
  _autoRunActive: boolean;
  _autoRunOptions: AnyRecord | null;

  constructor(partHistory: any = null, registry: any = null) {
  ...
  checkConstraintErrors(partHistory: any = this.partHistory, options: AnyRecord = {}) {
    ...
    const mutatePersistentData = (entry: ConstraintEntry, mutator: (helpers: AnyRecord) => void) => {
  ...
  async runAll(partHistory: any = this.partHistory, options: AnyRecord = {}) {
    ...
    const renderScene = () => {
      try { viewer?.render?.(); } catch { /* ignore render failures */ }
      try { viewer?.requestRender?.(); } catch { /* ignore render request failures */ }
    };
