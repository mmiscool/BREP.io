...
type TransformReferenceValue = string | TransformReferenceObject;

type SanitizedTransformValue = {
  position: Vec3Array;
  rotationEuler: Vec3Array;
  scale: Vec3Array;
  reference?: TransformReferenceValue;
};

type TransformReferenceBaseOptions = {
...
function sanitizeVec3(value, fallback = [0, 0, 0]): Vec3Array {
...
function sanitizeTransformReference(value): TransformReferenceValue | null {
  ...
  if (typeof value === 'object') {
    const name = normalizeReferenceName(value);
    if (!name) return null;
    const next: TransformReferenceObject = { name };
...
export function sanitizeTransformValue(value): SanitizedTransformValue {
  const source = value && typeof value === 'object' ? value : {};
  const next: SanitizedTransformValue = {
...
export function resolveTransformReferenceBase(
  reference,
  source,
  options: TransformReferenceBaseOptions = {},
  THREE = THREE_NS,
) {
  ...
  const referenceObject = typeof referenceMeta === 'object' && referenceMeta ? referenceMeta : null;
  const pickPoint = Array.isArray(referenceObject?.pickPoint) ? new THREE.Vector3(...referenceObject.pickPoint) : null;
  const object = resolveTransformReferenceObject(reference, source);
  const type = String(referenceObject?.type || object?.type || '').trim().toUpperCase();
...
export function composeReferencedTransformMatrix(
  transform,
  source,
  options: TransformReferenceBaseOptions = {},
  THREE = THREE_NS,
