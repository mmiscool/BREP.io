...
function runComponentDijkstra(graph, component: any, startId) {
  const nodeIds = Array.isArray(component?.nodeIds) ? component.nodeIds.map((nodeId) => String(nodeId || '')) : [];
  const pending = new Set<string>(nodeIds);
  const distances = new Map<string, number>(nodeIds.map((nodeId) => [nodeId, Number.POSITIVE_INFINITY]));
...
export function buildWireHarnessFormboardDefinition(partHistory, options: any = {}) {
...
export function insertWireHarnessFormboard(sheetManager, sheetIdOrIndex, partHistory, options: any = {}) {
