...
import type {Digraph, GraphNode} from "./digraph.js";
...
class NodeAndDistance {
  ...
  constructor(nodeId: string, distance: number, globalCounter = 0) {
    this.nodeId = nodeId;
...
export function _setUseMinHeap(value: boolean): void {
  _USE_MIN_HEAP = value;
}

function nodeAndDistanceLess(a: NodeAndDistance, b: NodeAndDistance): boolean {
...
class DistanceHeap {
  ...
  removeShortestDistanceNode(): NodeAndDistance | null {
    return this.heap.remove();
  }

  addNode(nodeAndDistance: NodeAndDistance): void {
...
class SimpleDistanceHeap {
  ...
  checkIntegrity(): void {
    // Map-backed debug heap has no structural invariant to verify.
  }
...
export class ShortestPathTree {
  ...
  parentNodeIds: Map<string, string>;
  parentEdgeIds: Map<string, string>;
...
//                    console.log("!!! " + childId + " := " + newDistance + ", from " + nodeId + ", " + counter);
  ...
  getDistance(endNodeId: string): number | undefined {
    return this.distances.get(endNodeId);
  }

  getShortestPath(endNodeId: string, resultSegmentsArray: string[], resultNodesArray: string[]): boolean {
    ...
    while (parentNodeId != null) {
      if (parentEdgeId != null) resultSegmentsArray.push(parentEdgeId);
  ...
  getShortestPathInformation(endNodeId: string): PathInformation {
  ...
  getAllShortestPathMap(): Map<string, PathInformation> {
    const results = new Map<string, PathInformation>();
...
export class PathInformation {
  ...
  constructor(
    startId: string | null | undefined = null,
    endId: string | null | undefined = null,
    pathSegments: string[] | null = null,
    pathNodes: string[] | null = null,
    distance?: number,
  ) {
    this.distance = distance;
