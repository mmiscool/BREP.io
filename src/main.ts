...
async function runBusyUiTask(message, run, options: AnyRecord = {}) {
...
async function renderHome() {
  ...
  workspaceRepoInput?.addEventListener('keydown', (event: any) => {
    if (event.key !== 'Enter') return;
    event.preventDefault();
    state.root.querySelector('[data-action="add-repo"]')?.dispatchEvent(new Event('click'));
  });

  tokenInput?.addEventListener('keydown', (event: any) => {
    if (event.key !== 'Enter') return;
    event.preventDefault();
...
async function resolveGithubBrowserBranch(entry) {
  const repoFull = String(entry?.repoFull || '').trim();
  if (!repoFull) return '';
  const cfg: AnyRecord = getGithubStorageConfig() || {};
...
function createFolderExplorer(roots, term = '') {
  ...
  widget = new WorkspaceFileBrowserWidget({
    ...
    createFolderActionsMenu: (entry, { tile = false, source, repoFull }: AnyRecord = {}) => createFolderActionsMenu(entry, {
      source: normalizeStorageSource(source || 'local'),
      repoFull: String(repoFull || '').trim(),
    }, {
      extraClass: tile ? 'hub-browser-tile-menu' : '',
...
function createFloatingActionsMenu(actions = [], options: AnyRecord = {}) {
...
function getTrashFolderPathsInScope(source, repoFull = '') {
  ...
  return (Array.from(new Set(paths)) as string[]).sort((a, b) => b.length - a.length);
}
...
function collectFolderSubtree(source, repoFull = '', folderPath = '') {
  ...
  return {
    records,
    folderPaths: Array.from(new Set(folderPaths)) as string[],
  };
}

async function deleteFolderPath(folderPathRaw, options: AnyRecord = {}) {
  if (isUiBusy()) return;
...
async function renameFolderPath(folderPathRaw, options: AnyRecord = {}) {
  if (isUiBusy()) return;
  const folderPath = normalizePath(folderPathRaw);
  if (!folderPath) return;
  const source = normalizeStorageSource(options?.source || state.explorerRootSource);
...
function getFileScopeOptions(source, repoFull = '', path = '') {
  const scope: AnyRecord = getFolderScopeOptions(source, repoFull);
...
async function runFileTransferAction(entry, { copy = true }: AnyRecord = {}) {
  if (isUiBusy()) return;
  ...
  const picked: AnyRecord = await openTransferBrowserDialog({
...
async function runFolderTransferAction(folderEntry, options: AnyRecord = {}) {
  if (isUiBusy()) return;
...
function createFolderActionsMenu(folderEntry, context: AnyRecord = {}, options: AnyRecord = {}) {
...
function createFileActionsMenu(entry, options: AnyRecord = {}) {
  const name = String(entry?.name || '').trim();
  const source = normalizeStorageSource(entry?.source);
  const repoFull = String(entry?.repoFull || '').trim();
  const branch = String(entry?.branch || '').trim();
