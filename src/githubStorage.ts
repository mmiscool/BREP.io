...
function _enqueueGithubWrite(key: any, op: any) {
...
function keyToPath(key: any, rootDir = STORAGE_ROOT) {
  if (isModelKey(key)) {
...
function pathToKey(path: any, rootDir = STORAGE_ROOT) {
...
function authHeaders(token: any) {
  return {
...
async function ghFetch(url: any, token: any, opts: any = {}) {
...
async function ghFetchAllPages(url: any, token: any) {
  let out = [];
  let next = url;
  while (next) {
...
export async function fetchGithubUserRepos(token: any) {
...
export function encodeRepoItemName(name: any) {
  return encodeURIComponent(String(name || ''));
}

export function decodeRepoItemName(name: any) {
  try { return decodeURIComponent(String(name || '')); } catch { return String(name || ''); }
}

export async function listGithubDir({ token, repoFull, branch, path }: any) {
  const t = String(token || '').trim();
  if (!t) return [];
...
export async function listGithubRepoTree({ token, repoFull, branch }: any) {
  const t = String(token || '').trim();
...
export async function readGithubFileBase64({ token, repoFull, branch, path }: any) {
  const t = String(token || '').trim();
  if (!t) return null;
  const { owner, repo } = parseRepo(repoFull);
  const url = new URL(`${GH.apiBase}/repos/${owner}/${repo}/contents/${encodePath(path)}`);
...
export async function writeGithubFileBase64({ token, repoFull, branch, path, base64, message, retryOn409 = 2 }: any) {
  const t = String(token || '').trim();
...
export async function deleteGithubFile({ token, repoFull, branch, path, message }: any) {
  const t = String(token || '').trim();
  if (!t) return;
  const { owner, repo } = parseRepo(repoFull);
  let sha = null;
...
async function readGithubFileMeta({ token, repoFull, branch, path }: any) {
  const t = String(token || '').trim();
  if (!t) return null;
  const { owner, repo } = parseRepo(repoFull);
  const url = new URL(`${GH.apiBase}/repos/${owner}/${repo}/contents/${encodePath(path)}`);
...
export class GithubStorage {
  ...
  async _readFileContent(path: any, meta: any) {
    let file = meta;
  ...
  async _loadFromRepo(session: any) {
  ...
  async _writeKeyToRepo(key: any, value: any) {
  ...
  async _removeKeyFromRepo(key: any) {
    if (!this._token || !this._repo) return;
    const { owner, repo } = this._repo;
    const path = keyToPath(key, this._rootDir);
