...
export class SelectionFilter {
    ...
    static allowedSelectionTypes: any = SelectionFilter.ALL;
    static viewer: any = null;
    static previouseAllowedSelectionTypes: any = null;
    static _hovered: Set<any> = new Set(); // objects currently hover-highlighted
    static _hoveredSourceMap: Map<any, any> = new Map(); // key -> source object for hover
    static hoverColor = '#fbff00'; // default hover tint
    static _selectionActions: Map<any, any> = new Map();
    static _selectionActionOrder: any[] = [];
    static _selectionActionSeq = 1;
    static _selectionActionListenerBound = false;
    static _selectionActionsPending = false;
    static _selectionActionBar: any = null;
    static _historyContextActions: Map<any, any> = new Map();
    static _selectionActionSeparator: any = null;
    static _contextSuppressReasons: Set<any> = new Set();
    static _selectionFilterIndicator: any = null;
    static _selectionFilterIndicatorToggle: any = null;
    static _selectionFilterIndicatorPanel: any = null;
    static _selectionFilterCheckboxes: Map<any, any> = new Map();
    static _selectionFilterTypes: any = null;
    static _selectionFilterOutsideBound = false;
    static _selectionFilterTintBtn: any = null;
    static _selectionFilterAllToggleBtn: any = null;
    static _clickWatcherTimer: any = null;
    static _missingClickLogged: Map<any, any> = new Map();
    static _clickWatcherIntervalMs = 2000;
    static _onClickWatcherSeq = 1;
    static _uiCallback: any = null;
    static _selectableTintState: any = {
        active: false,
        activeColor: null,
        colorIndex: 0,
    ...
    static _withSilentOnClick(target, fn) {
        ...
        try { target.__brepOnClickSilent = true; } catch { /* ignore */ }
        try { fn(); } catch { /* ignore */ } finally {
            try { target.__brepOnClickSilent = false; } catch { /* ignore */ }
        }
    }
    ...
    static _installOnClickWatcher(target) {
        ...
        if (existingDesc?.get && (existingDesc.get as any).__brepOnClickWatcher) return;
        let current = typeof target.onClick !== 'undefined' ? target.onClick : undefined;
        const getter = function () { return current; };
        (getter as any).__brepOnClickWatcher = true;
        ...
        try {
            ...
            try { target.onClick = current; } catch { /* ignore */ }
        }
    }
    ...
    static setHoverColor(hex) {
        if (!hex) return;
        try { SelectionFilter.hoverColor = String(hex); } catch (_) { /* ignore */ }
    ...
    static setHoverObject(obj, options: any = {}) {
        SelectionFilter.setHoverObjects(obj ? [obj] : [], options);
    }

    static setHoverObjects(objs, options: any = {}) {
    ...
    static setHoverByName(scene, name, options: any = {}) {
    ...
    static #handleReferenceSelection(objectToToggleSelectionOn, context: any = {}) {
        try {
            ...
            if (!usable) {
                try { activeRefInput.removeAttribute('active-reference-selection'); } catch (_) { /* ignore */ }
                try { activeRefInput.style.filter = 'none'; } catch (_) { /* ignore */ }
                try {
                ...
                try { if (window.__BREP_activeRefInput === activeRefInput) window.__BREP_activeRefInput = null; } catch (_) { /* ignore */ }
                ...
                if (hasMax && currentCount >= maxSelections) {
                    try {
                        ...
                        if (wrap) {
                            ...
                            setTimeout(() => {
                                try { wrap.classList.remove('ref-limit-reached'); } catch (_) { /* ignore */ }
                            }, 480);
                        }
                    } catch (_) { /* ignore */ }
                    return true;
            ...
            if (!keepActive) {
                ...
                try { window.__BREP_activeRefInput = activeRefInput; } catch (_) { /* ignore */ }
            }
            return true;
        } catch (error) {
            if (debugMode) {
    ...
    static selectItem(scene, itemName, options: any = {}) {
    ...
    static triggerUI() {
        ...
        try { SelectionFilter._updateSelectionFilterIndicator(); } catch (_) { /* ignore */ }
    }
    ...
    static getSelectedObjects(options: any = {}) {
    ...
    static registerSelectionAction(spec: any = {}) {
        if (!spec) return null;
    ...
    static unregisterSelectionAction(id) {
        ...
        if (entry?.btn && entry.btn.parentNode) {
            try { entry.btn.parentNode.removeChild(entry.btn); } catch { /* ignore */ }
        }
    ...
    static _syncSelectionActions() {
        ...
        if (suppressed) {
            try { bar.style.display = 'none'; } catch { /* ignore */ }
            return;
        }
        const selection = SelectionFilter.getSelectedObjects();
        ...
        try { bar.textContent = ''; } catch { /* ignore */ }
        for (const btn of utilityButtons) {
            try { bar.appendChild(btn); } catch { /* ignore */ }
        }
        if (utilityButtons.length && contextButtons.length) {
            const sep = SelectionFilter._ensureSelectionActionSeparator();
            if (sep) {
                try { bar.appendChild(sep); } catch { /* ignore */ }
            }
        }
        for (const btn of contextButtons) {
            try { bar.appendChild(btn); } catch { /* ignore */ }
        }
        try { bar.style.display = (utilityButtons.length + contextButtons.length) > 0 ? 'flex' : 'none'; } catch { /* ignore */ }
    }
    ...
    static async _createFeatureFromContext(viewer, typeKey, params = null) {
        ...
        if (viewer.historyWidget && typeof viewer.historyWidget._handleAddEntry === 'function') {
            try { entry = await viewer.historyWidget._handleAddEntry(typeKey); } catch { /* ignore */ }
        } else {
            try { entry = await viewer.partHistory?.newFeature?.(typeKey); } catch { /* ignore */ }
        }
    ...
    static _createConstraintFromContext(viewer, typeKey, params = null) {
        ...
        SelectionFilter.setContextBarSuppressed('context-create', true);
        setTimeout(() => SelectionFilter.setContextBarSuppressed('context-create', false), 0);
        try { viewer.partHistory?.assemblyConstraintHistory?.addConstraint?.(typeKey, params || null); } catch { /* ignore */ }
    }
    ...
    static _createAnnotationFromContext(viewer, typeKey, params = null) {
        ...
        SelectionFilter.setContextBarSuppressed('context-create', true);
        setTimeout(() => SelectionFilter.setContextBarSuppressed('context-create', false), 0);
        try { viewer._pmiMode?._annotationHistory?.createAnnotation?.(typeKey, params || null); } catch { /* ignore */ }
    }
    ...
    static _applyContextParamsToEntry(viewer, entry, params = {}) {
        ...
        try {
            ...
            if (!refresh()) {
                setTimeout(() => { try { refresh(); } catch { /* ignore */ } }, 0);
            }
        } catch { /* ignore */ }
    }
    ...
    static _applySelectableTint(scene, { allowFace, allowEdge, faceColor, edgeColor }) {
        ...
        const storeColor = (mat) => {
            ...
            try { state.materials.set(mat, `#${mat.color.getHexString()}`); } catch { /* ignore */ }
        };
        const tintMaterial = (mat, color) => {
            if (!mat || !mat.color || typeof mat.color.set !== 'function') return;
            storeColor(mat);
            try { mat.color.set(color); } catch { /* ignore */ }
    ...
    static _restoreSelectableTint() {
        ...
        SelectionFilter._updateSelectableTintButton();
        try { SelectionFilter.viewer?.render?.(); } catch { /* ignore */ }
    }
    ...
    static _toggleSelectableTint(viewer) {
        ...
        SelectionFilter._updateSelectableTintButton();
        try { (viewer || SelectionFilter.viewer)?.render?.(); } catch { /* ignore */ }
    }
    ...
    static _ensureSelectionFilterIndicator(viewer) {
        ...
        allToggleBtn.addEventListener('click', (ev) => {
            ...
            try { SelectionFilter.SetSelectionTypes(enableAll ? SelectionFilter.ALL : []); } catch { /* ignore */ }
            if (SelectionFilter.previouseAllowedSelectionTypes !== null) {
                SelectionFilter.previouseAllowedSelectionTypes = SelectionFilter.allowedSelectionTypes;
            }
            SelectionFilter._updateSelectionFilterIndicator();
        ...
        for (const type of types) {
            ...
            box.addEventListener('change', (ev) => {
                ...
                try { SelectionFilter.SetSelectionTypes(nextValue); } catch { /* ignore */ }
                if (SelectionFilter.previouseAllowedSelectionTypes !== null) {
                    SelectionFilter.previouseAllowedSelectionTypes = SelectionFilter.allowedSelectionTypes;
                }
                SelectionFilter._updateSelectionFilterIndicator();
    ...
    static _createSelectionActionButton(entry) {
        try {
            ...
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                try { btn.__sabOnClick && btn.__sabOnClick(); } catch { /* ignore */ }
            });
            const isIcon = String(entry?.label || '').length <= 2;
