...
export class FloatingWindow {
  ...
  static _activeModal: any = null;
  static _modalOpenCount: any = 0;
  ...
  constructor({
    ...
    try { window.addEventListener('resize', this._onViewportResize, { passive: true }); } catch { /* ignore floating window fallback failures */ }
    ...
    header.addEventListener('dragstart', (e) => { try { e.preventDefault(); } catch { /* ignore floating window fallback failures */ } });
    ...
    if (modalOverlay) {
      modalOverlay.addEventListener('pointerdown', (ev) => {
        ...
        try { ev.preventDefault(); ev.stopPropagation(); } catch { /* ignore floating window fallback failures */ }
        this.close();
      });
      this._activateModal();
    }
  ...
  destroy() {
    ...
    try { window.removeEventListener('resize', this._onViewportResize); } catch { /* ignore floating window fallback failures */ }
    if (this._viewportResizeTimer) {
      try { window.clearTimeout?.(this._viewportResizeTimer); } catch { /* ignore floating window fallback failures */ }
      this._viewportResizeTimer = 0;
    }
  ...
  focus() {
    ...
    try { target?.focus?.({ preventScroll: true }); } catch { try { target?.focus?.(); } catch { /* ignore floating window fallback failures */ } }
  }
  bringToFront() { this._bringToFront(); }
  setTransparent(transparent: any) {
  ...
  _getMaxZIndex(excludeEl: any = null) {
  ...
  _buildTransparencyButton() {
    ...
    btn.addEventListener('click', (ev) => {
      try { ev.stopPropagation(); } catch { /* ignore floating window fallback failures */ }
  ...
  _onHeaderPointerDown(ev: any) {
    ...
    const onUp = (e: any) => {
      this._dragging = false;
      try { this.header.releasePointerCapture?.(ev.pointerId); } catch { /* ignore floating window fallback failures */ }
  ...
  _onResizerPointerDown(ev: any, dir = 'se') {
    if (ev.button !== 0) return;
    this._resizing = true;
    ...
    const onUp = (_e) => {
      this._resizing = false;
      try { resizerEl?.releasePointerCapture?.(ev.pointerId); } catch { /* ignore floating window fallback failures */ }
      window.removeEventListener('pointermove', onMove, true);
      window.removeEventListener('pointerup', onUp, true);
  ...
  _activateModal() {
    ...
    try { document.addEventListener('keydown', this._onModalKeyDown, true); } catch { /* ignore floating window fallback failures */ }
  }
  ...
  _deactivateModal() {
    ...
    FloatingWindow._modalOpenCount = Math.max(0, (FloatingWindow._modalOpenCount || 0) - 1);
    FloatingWindow._syncGlobalDialogFlag();
    try { document.removeEventListener('keydown', this._onModalKeyDown, true); } catch { /* ignore floating window fallback failures */ }
    if (this._restoreFocus && this._previousFocus && this._previousFocus !== document.body) {
      try { this._previousFocus.focus?.({ preventScroll: true }); } catch { try { this._previousFocus.focus?.(); } catch { /* ignore floating window fallback failures */ } }
    }
    this._previousFocus = null;
  }

  _handleModalKeyDown(ev: any) {
    if (!this._modal || !this._modalActive || !this._isVisible()) return;
    ...
    if (!focusable.length) {
      try { ev.preventDefault(); this.focus(); } catch { /* ignore floating window fallback failures */ }
      return;
    }
    ...
    if (ev.shiftKey && (!active || active === first || !this.root.contains(active))) {
      ev.preventDefault();
      try { last.focus(); } catch { /* ignore floating window fallback failures */ }
    } else if (!ev.shiftKey && active === last) {
      ev.preventDefault();
      try { first.focus(); } catch { /* ignore floating window fallback failures */ }
    }
  }
  ...
  static _syncGlobalDialogFlag() {
    try {
      ...
      const hasLegacyDialog = typeof (window as any).isDialogOpen === 'function' && (window as any).isDialogOpen();
      (window as any).__BREPDialogOpen = hasFloatingModal || hasLegacyDialog;
    } catch { /* ignore floating window fallback failures */ }
  }
}
