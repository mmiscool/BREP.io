...
class ShareModelPanel {
  ...
  linkEl: HTMLTextAreaElement | null;
  iframeEl: HTMLTextAreaElement | null;
  copyLinkBtn: HTMLButtonElement | null;
  copyIframeBtn: HTMLButtonElement | null;
  openLinkBtn: HTMLButtonElement | null;
  refreshBtn: HTMLButtonElement | null;
  modeCadBtn: HTMLButtonElement | null;
  modeViewerBtn: HTMLButtonElement | null;
  shareUrl: string;
  iframeHtml: string;
  shareMode: string;
  _lastGithubTarget: string;
...
export function createShareButton(viewer) {
  ...
  if (!targetViewer[PANEL_KEY]) targetViewer[PANEL_KEY] = new ShareModelPanel(targetViewer);
  return {
    label: '⠪',
    title: 'Share current model',
    onClick: () => targetViewer[PANEL_KEY].toggle(),
  };
}
