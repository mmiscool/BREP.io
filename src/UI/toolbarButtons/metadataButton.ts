...
class MetadataPanelController {
    ...
    currentTarget: AnyRecord | null;
    _cachedInputHeightPx: number | null;
    ...
    _render() {
        ...
        const appendMetadataBlock = (labelText, metadataValue, options: AnyRecord = {}) => {
...
export function createMetadataButton(viewer) {
    ...
    if (!targetViewer[PANEL_CONTROLLER]) {
        targetViewer[PANEL_CONTROLLER] = new MetadataPanelController(targetViewer);
    }
    const controller = targetViewer[PANEL_CONTROLLER];
    const onClick = () => {
        controller.toggle();
    };
    return {
