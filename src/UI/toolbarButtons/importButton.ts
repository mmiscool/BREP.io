        ...
        try {
          ...
          if (!isZip && isJSON) {
            ...
            try {
              ...
              // invalid JSON; fall through to part history import
            }
          ...
          if (isZip) {
            ...
            if (fhKey) {
              ...
              try { root = JSON.parse(jsonData); } catch {
                // invalid embedded history; fall through to raw import
              }
              // Ensure expressions is a string if present
              if (root && root.expressions != null && typeof root.expressions !== 'string') {
                try { root.expressions = String(root.expressions); } catch {
                  root.expressions = String(root.expressions);
                }
              }
              if (root) {
                await viewer?.partHistory?.reset?.();
