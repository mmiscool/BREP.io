...
function _buildOBJColorContext(solid, mesh, metadataManager) {
  ...
  const getMetadata = (name) => {
    ...
    try { return metadataManager.getMetadata(name); } catch {
      return null;
    }
  };
  ...
  if (faceIDs && idToFaceName) {
    ...
    for (let t = 0; t < faceIDs.length; t++) {
      ...
      let faceMeta = null;
      try { faceMeta = (typeof solid?.getFaceMetadata === 'function') ? solid.getFaceMetadata(faceName) : null; } catch {
        faceMeta = null;
      }
    ...
    try {
      ...
      if (fmt === 'json') {
        try {
          ...
          // fall through to show alert below
        }
      }
