...
class SolidOverlapDiagnosticsPanel {
  constructor(viewer) {
    ...
    try { this.viewer.__solidOverlapDiagnosticsController = this; } catch { /* ignore */ }
    try { window.addEventListener('selection-changed', this._selectionChanged); } catch { /* ignore */ }
  }
  ...
  _renderAnalysis() {
    ...
    makeSection(
      'Highlighted faces',
      String(Object.values(analysis.highlightedBySolid || {}).reduce((sum, record) => sum + (record?.faceNames?.length || 0), 0)),
    );
    makeSection(
      'Tolerance',
  ...
  _ensureOverlayGroup() {
    ...
    let group = null;
    try { group = scene.getObjectByName(OVERLAY_GROUP_NAME) || null; } catch { group = null; }
    if (!group) {
  ...
  _clearOverlayChildren() {
    ...
    for (const mesh of this._overlayMeshes.splice(0)) {
      try { group.remove(mesh); } catch { /* ignore */ }
...
export function createSolidOverlapDiagnosticsButton(viewer) {
  ...
  if (!viewer[PANEL_KEY]) {
    viewer[PANEL_KEY] = new SolidOverlapDiagnosticsPanel(viewer);
  }
