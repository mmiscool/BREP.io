...
export const cameraMethods = {
    ...
    _syncActiveTransformGizmosForCamera({ resetSize = false } = {}) {
        ...
        try {
            const globalState = (typeof window !== 'undefined') ? (window as any).__BREP_activeXform : null;
    ...
    _computeSceneBounds({ reuse = false, includeExcluded = false } = {}) {
        ...
        try { this.scene.updateMatrixWorld(true); } catch { /* ignore stale scene transform failures */ }
                ...
                if (geom) {
                    ...
                    if (obj.boundingBox !== undefined) {
                        if (obj.boundingBox == null && typeof obj.computeBoundingBox === 'function') {
                            try { obj.computeBoundingBox(); } catch { /* ignore object bounds failures */ }
                        }
                        bbox = obj.boundingBox;
                    } else {
                        if (geom.boundingBox == null && typeof geom.computeBoundingBox === 'function') {
                            try { geom.computeBoundingBox(); } catch { /* ignore geometry bounds failures */ }
                        }
    ...
    zoomToFit(margin = 1.1) {
        try {
            ...
            if (this.camera.isOrthographicCamera) {
                ...
                try { c.updateMatrixState && c.updateMatrixState(); } catch { /* ignore controls state sync failures */ }
            ...
            // Sync and render
            try { c.update && c.update(); } catch { /* ignore controls update failures */ }
            this.render();
        } catch { /* noop */ }
    },
    ...
    _resizeRendererToDisplaySize() {
        ...
        if (isWebGL) {
            ...
            } catch { /* ignore renderer element style failures */ }
        }

        // Keep fat-line materials in sync with canvas resolution
        try {
            const setRes = (mat: any) => mat && mat.resolution && typeof mat.resolution.set === 'function' && mat.resolution.set(width, height);
        ...
        } catch { /* ignore global material resolution sync failures */ }
        ...
        } catch { /* ignore per-object material resolution sync failures */ }
    ...
    _onResize() {
        ...
        requestAnimationFrame(() => {
            ...
            try { this._sketchMode?.onCameraChanged?.(); } catch { /* ignore sketch overlay resize sync failures */ }
        });
    },
    ...
    _onControlsChange() {
        ...
        if (this._shouldSuppressSceneHover()) {
            try { SelectionFilter.clearHover(); } catch { /* ignore hover clear failures */ }
        ...
        } catch { /* ignore dash spacing sync failures */ }
        // While orbiting/panning/zooming, reposition dimension labels/leaders
        try { this._sketchMode?.onCameraChanged?.(); } catch { /* ignore sketch overlay camera sync failures */ }
    },
    ...
    _updateOverlayDashSpacing(width, height) {
        ...
        const setDash = (mat: any) => {
            if (!mat) return;
            try {
                mat.dashSize = dashPx * wpp;
                mat.gapSize = gapPx * wpp;
                mat.needsUpdate = true;
            } catch { /* ignore unsupported dashed material fields */ }
        };
        try {
            const edges: any = CADmaterials?.EDGE || {};
            setDash(edges.OVERLAY);
            setDash(edges.THREAD_SYMBOLIC_MAJOR);
        } catch { /* ignore dash material sync failures */ }
    }
};
