    ...
    _updateAxisHelpers() {
        ...
        for (const helper of this._axisHelpers) {
            ...
            if (compensate && helper.parent && typeof helper.parent.getWorldScale === 'function') {
                try { helper.parent.updateMatrixWorld?.(true); } catch { /* ignore */ }
        ...
        const applyMaterial = (obj, baseMaterial, color) => {
            ...
            if (!color) {
                if (ud.__metadataMaterial && ud.__metadataMaterial !== defaultMaterial) {
                    try { ud.__metadataMaterial.dispose?.(); } catch { /* ignore */ }
                }
                try { delete ud.__metadataMaterial; } catch { /* ignore */ }
                try { delete ud.__metadataColor; } catch { /* ignore */ }
            ...
            try { nextMat.needsUpdate = true; } catch { /* ignore */ }
