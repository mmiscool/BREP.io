...
type FaceObjectLike = THREE.Object3D & {
  geometry?: THREE.BufferGeometry;
  userData?: Record<string, any>;
  getAverageNormal?: () => THREE.Vector3 | null | undefined;
};

type MaybeFaceObject = FaceObjectLike | null | undefined;

export function isFaceObject(object: MaybeFaceObject): boolean {
  if (!object) return false;
  const type = object.userData?.type || object.userData?.brepType || object.type;
  return String(type).toUpperCase() === 'FACE';
}

export function computeFaceOrigin(object: MaybeFaceObject): THREE.Vector3 | null {
...
export function computeFaceCenter(object: MaybeFaceObject): THREE.Vector3 | null {
  if (!object) return null;
...
export function computeFaceNormal(object: MaybeFaceObject): THREE.Vector3 | null {
  if (!object) return null;
  try {
...
export function estimateArrowLength(object: MaybeFaceObject): number {
  const geom = object?.geometry;
...
function fallbackQuaternionNormal(object: MaybeFaceObject): THREE.Vector3 | null {
  try {
