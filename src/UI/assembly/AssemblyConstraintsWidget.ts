...
export class AssemblyConstraintsWidget {
  ...
  async #waitForIterationContinue(run) {
    ...
    run.continueDeferred.promise = new Promise<void>((resolve) => {
  ...
  _setConstraintGraphicsEnabled(enabled, options: any = {}) {
  ...
  _applyConstraintHighlight(entry, ConstraintClass, options: any = {}) {
    ...
    const refFields = Object.entries(schema as any).filter(([, def]: any) => def?.type === 'reference_selection');
  ...
  #collectConstraintReferenceSelections(entry, { limit = Infinity } = {}) {
    ...
    const refKeys = Object.entries(schema as any)
      .filter(([, def]: any) => def?.type === 'reference_selection')
      .map(([key]) => key);
  ...
  #upsertConstraintArrows(constraintID, arrowSpecs = [], color = DEFAULT_CONSTRAINT_COLOR) {
    ...
    const prevChildren: any[] = Array.from(group.children || []);
  ...
  #upsertConstraintVisual(constraintID, visual: any = {}, color = DEFAULT_CONSTRAINT_COLOR) {
    const segments = Array.isArray(visual?.segments) ? visual.segments : [];
  ...
  #beginAngleLabelDrag(constraintID, entry, event, constraintClass = null) {
    ...
    const visual: any = this.#constraintVisualData(entry, cls);
