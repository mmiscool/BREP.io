...
export class Sheet2DEditorWindow {
  ...
  _syncZoomControl() {
    ...
    const hasPreset = (Array.from(this._zoomSelect.options) as any[]).some((option) => option.value === exactValue);
    if (!hasPreset) {
  ...
  _getClipboardImagePayload(event) {
    const clipboard = event?.clipboardData || window.clipboardData || null;
    const items = Array.from(clipboard?.items || []) as any[];
  ...
  _pasteIntoSelectedTable(pastedTableData) {
    ...
    let next: any = ensureTableSize(this._getNormalizedTableData(element), targetRect.maxRow + 1, targetRect.maxCol + 1);
    ...
    for (let row = targetRect.minRow; row <= targetRect.maxRow; row += 1) {
      for (let col = targetRect.minCol; col <= targetRect.maxCol; col += 1) {
        next.cells[row][col] = { text: "", rowSpan: 1, colSpan: 1, mergedInto: null, style: normalizeTableCellStyle({}) };
      }
    }
  ...
  _readBlobAsDataUrl(blob): Promise<string> {
  ...
  _measureImageInsertSizeIn(src): Promise<any> {
  ...
  async _pasteClipboardImage(payload: any) {
    if (!this.sheetDraft) return false;
  ...
  _renderSelectionToolbar() {
    ...
    const formboardTextOnly = !!(textState as any)?.isFormboard;
  ...
  _drawPdfSvgTextNode(doc, node, style, context) {
    ...
    const segments = [];
    const tspans = Array.from(node?.querySelectorAll?.(":scope > tspan") || []) as any[];
    if (tspans.length) {
  ...
  _drawPdfFramedRect(doc, x, y, w, h, { fill, stroke, radius = (8 / 96) }: any = {}) {
  ...
  _buildSelectionOverlay(target) {
    ...
    const renderedNode = targetId
      ? (Array.from(this._slideCanvas?.querySelectorAll?.(".sheet-slides-element") || []) as any[])
  ...
  _appendShapeSurface(host, element, widthPx, heightPx, options: any = {}) {
  ...
  _renderInspector() {
    ...
    const textState = selectionCount === 1 ? this._getSelectedTextState() : null;
    const supportsText = !!textState;
    const supportsFormboardText = !!(textState as any)?.isFormboard;
    ...
    (Object.values(this._textAlignButtons || {}) as any[]).forEach((button) => { button.disabled = !supportsText || supportsFormboardText; });
    (Object.values(this._verticalAlignButtons || {}) as any[]).forEach((button) => { button.disabled = !supportsText || supportsFormboardText; });
    ...
    if (!hasElement) {
      ...
      (Object.values(this._textAlignButtons || {}) as any[]).forEach((button) => button.classList.remove("primary"));
      (Object.values(this._verticalAlignButtons || {}) as any[]).forEach((button) => button.classList.remove("primary"));
    ...
    if (supportsText) {
      ...
      const textAlign = String(textState.textAlign || "left");
      const verticalAlign = String(textState.verticalAlign || "middle");
      (Object.entries(this._textAlignButtons || {}) as any[]).forEach(([value, button]) => {
        button.classList.toggle("primary", value === textAlign);
      });
      (Object.entries(this._verticalAlignButtons || {}) as any[]).forEach(([value, button]) => {
