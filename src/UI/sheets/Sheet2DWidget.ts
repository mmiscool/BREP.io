...
function toSheetSummary(sheet: any) {
...
export class Sheet2DWidget {
  ...
  dispose() {
    if (typeof this._removeManagerListener === "function") {
      try { this._removeManagerListener(); } catch { /* ignore listener cleanup failure */ }
    }
    this._removeManagerListener = null;
  ...
  _bindManagerListener() {
    ...
    this._removeManagerListener = manager.addListener((sheets: any) => {
  ...
  _openSheet(sheetId) {
    ...
    this._renderList();
    try { this.viewer?.openSheet2DEditor?.(this._activeSheetId); } catch { /* ignore editor open failure */ }
  }
  ...
  _moveSheet(sheetId: any, toIndex: any) {
    const manager = this._getManager();
    const id = String(sheetId || "").trim();
  ...
  _renderList() {
    ...
    for (const sheet of this.sheets) {
      ...
      if (!this._readOnly) {
        ...
        row.addEventListener("dragleave", (event: any) => {
