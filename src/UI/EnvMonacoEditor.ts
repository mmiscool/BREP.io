...
class EnvMonacoEditor extends HTMLElement {
  [key: string]: any;
  static _monacoPromise: any;
  static _runtimeCompletionRegistered: boolean;
  ...
  async _boot() {
    try {
      ...
      } catch { /* ignore loading status update failures */ }
      try { console.warn('[EnvMonacoEditor] Failed to load Monaco', e); } catch { /* ignore console failures */ }
    }
  }
  ...
  _collectProps(obj) {
    ...
    while (cursor && cursor !== Object.prototype && cursor !== Function.prototype) {
      ...
      for (const [key, desc] of Object.entries(descs) as [string, PropertyDescriptor][]) {
  ...
  static _registerRuntimeCompletion(monaco) {
    ...
    } catch { /* ignore runtime completion registration failures */ }
  }
