...
export class MainToolbar {
  ...
  _btn(label, title, onClick) {
    ...
    b.textContent = label;
    b.title = title || label;
    (b as any).__mtbOnClick = onClick;
    b.addEventListener('click', (e) => { e.stopPropagation(); try { (b as any).__mtbOnClick && (b as any).__mtbOnClick(); } catch { /* ignore toolbar button handler errors */ } });
    return b;
  }

  // Public: allow plugins to add custom buttons to the left cluster
  addCustomButton(spec: any = {}) {
    try {
  ...
  reserveRightSpaceForElement(key, element, { extraPx = 12, minPx = 0 }: any = {}) {
    if (!key) return () => {};
    this._teardownRightReserveWatcher(key);
