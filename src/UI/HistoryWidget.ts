...
export class HistoryWidget extends HistoryCollectionWidget {
  ...
  #afterPartHistoryMutated({ forceRender = false, syncOpenForms = true }: any = {}) {
  ...
  async #yieldForHistoryRunUi({ force = false }: any = {}) {
  ...
  #handleFeatureDimensionFieldFocus(payload: any = {}) {
    const entryId = payload?.entryId != null ? String(payload.entryId) : null;
    const fieldKey = payload?.fieldKey != null ? String(payload.fieldKey) : null;
    if (!entryId) return;
  ...
  #decorateEntryHeader(context: any = {}) {
  ...
  #decorateMissingFeaturePanel(entry, context: any = {}) {
