...
type AccordionCallback = (section: AccordionSection) => void;
...
class AccordionSection {
  title: string;
  callbackOpen: AccordionCallback | null;
  callbackClose: AccordionCallback | null;
   ...
   */
  constructor({ title, titleElement, contentElement }: AccordionSectionOptions) {
    this.title = title;
...
export class AccordionWidget {
   ...
   * @param {string} title
   */
  async removeSection(title: string): Promise<boolean> {
   ...
   */
  async collapseAll(): Promise<boolean> {
   ...
   */
  async expandAll(): Promise<boolean> {
    const els = this.uiElement.querySelectorAll(".accordion-content");
   ...
   * @param {string} title
   */
  async expandSection(title: string): Promise<boolean> {
   ...
   */
  hideSection(title: string): boolean {
   ...
   * @returns {boolean} true if found
   */
  showSection(title: string): boolean {
    const titleEl = this._findTitleEl(title);
    const contentEl = this._findContentEl(title, titleEl);
    let changed = false;
    if (titleEl) {
  ...
  _findTitleEl(title: string): HTMLElement | null {
    const direct = this.uiElement.querySelector<HTMLElement>(`.accordion-title[name="accordion-title-${title}"]`);
    if (direct) return direct;
    const titles = Array.from<HTMLElement>(this.uiElement.querySelectorAll('.accordion-title'));
  ...
  _findContentEl(title: string, titleEl: HTMLElement | null = null): HTMLElement | null {
    const direct = this.uiElement.querySelector<HTMLElement>(`.accordion-content[name="accordion-content-${title}"]`);
    if (direct) return direct;
    if (titleEl && titleEl.nextElementSibling instanceof HTMLElement && titleEl.nextElementSibling.classList.contains('accordion-content')) {
      return titleEl.nextElementSibling;
    }
    // Fallback: find content by id match start
    const contents = Array.from<HTMLElement>(this.uiElement.querySelectorAll('.accordion-content'));
    const norm = String(title || '').trim().toUpperCase();
...
async function _flushAfterPaint(): Promise<void> {
  await _nextFrame();
  await _nextFrame();
}
