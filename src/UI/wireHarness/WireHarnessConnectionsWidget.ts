...
export class WireHarnessConnectionsWidget {
  ...
  _buildUI() {
    ...
    if (this._readOnly) {
      ...
      header.classList.add('wire-harness-widget-header-actions-only');
    }
  ...
  _renderList() {
    ...
    const routedCount = Array.from(this.routeResults.values() as Iterable<any>).filter((entry) => entry?.feasible).length;
    ...
    for (const label of ['ID', 'Length', 'Dia', 'From', 'To', 'Status', 'Actions']) {
  ...
  _ensureStyles() {
    ...
    style.textContent = `
      ...
      .wire-harness-widget-header-actions-only {
