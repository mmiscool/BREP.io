 ...
 */
export function generateObjectUI(target, options: any = {}) {
  ensureStyles();
  const cfg = {
      ...
      if (k.startsWith('_lazy') && typeof childVal === 'function') {
        ...
        const loadLazyValue = () => {
          ...
          try {
            ...
            if (actualNode instanceof HTMLDetailsElement) actualNode.open = true;
...
function makeReferenceEditor(refInfo, value, cfg) {
  ...
  btn.addEventListener('click', (event) => {
    try { event.preventDefault(); } catch { /* ignore blocked event prevention */ }
    try { event.stopPropagation(); } catch { /* ignore blocked propagation stop */ }
    try { cfg.onReferenceNavigate(refInfo); } catch { /* ignore reference navigation errors */ }
  });
  return btn;
}
