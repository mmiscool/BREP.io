...
const appWindow = typeof window !== 'undefined' ? window as any : {};
let _dialogOpenCount = 0;
const markDialogOpen = () => {
    _dialogOpenCount += 1;
    appWindow.__BREPDialogOpen = _dialogOpenCount > 0;
};
const markDialogClosed = () => {
    _dialogOpenCount = Math.max(0, _dialogOpenCount - 1);
    appWindow.__BREPDialogOpen = _dialogOpenCount > 0;
};
appWindow.isDialogOpen = () => _dialogOpenCount > 0;


appWindow.confirm = async (message, timeoutInSeconds = null, defaultValue = true) => {
    return new Promise<boolean>((resolve) => {
        ...
        const cleanup = () => {
            ...
            try { overlay.remove(); } catch (_) { try { dialog.remove(); } catch (_) { /* ignore dialog removal failures */ } }
            markDialogClosed();
        };
...
appWindow.alert = async (message, timeoutInSeconds = null) => {
...
appWindow.prompt = async (message, defaultValue = '') => {
