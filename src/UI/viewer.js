...
const SPACEMOUSE_VENDOR_IDS = new Set([0x046d, 0x256f]);
const SPACEMOUSE_NAME_HINTS = [
    'spacemouse',
    'space mouse',
    'spacenavigator',
    'space navigator',
...
function logSpaceMouse(...args) {
    try { console.log('[SpaceMouse]', ...args); } catch { }
}

function isSpaceMouseDevice(device) {
    if (!device) return false;
    const vendorId = Number(device.vendorId);
    if (Number.isFinite(vendorId) && SPACEMOUSE_VENDOR_IDS.has(vendorId)) return true;
    const name = String(device.productName || '').toLowerCase();
    return SPACEMOUSE_NAME_HINTS.some((hint) => name.includes(hint));
}

function describeHidDevice(device) {
    if (!device) return null;
    const collections = Array.isArray(device.collections)
        ? device.collections.map((c) => ({
            usagePage: c?.usagePage,
            usage: c?.usage,
            inputReports: Array.isArray(c?.inputReports) ? c.inputReports.length : 0,
            outputReports: Array.isArray(c?.outputReports) ? c.outputReports.length : 0,
            featureReports: Array.isArray(c?.featureReports) ? c.featureReports.length : 0,
        }))
        : null;
    return {
...
function normalizeSpaceMouseAxis(value, deadzone, max) {
    const abs = Math.abs(value);
    if (abs <= deadzone) return 0;
    const span = Math.max(1, max - deadzone);
    const scaled = (abs - deadzone) / span;
...
export class Viewer {
    ...
    constructor({
        ...
        // SpaceMouse/WebHID state (optional)
        this._spaceMouse = null;
    ...
    // SpaceMouse / WebHID
    // ----------------------------------------
    async requestSpaceMouse() {
        logSpaceMouse('Requesting WebHID device...');
        if (typeof navigator === 'undefined' || !navigator.hid) {
            logSpaceMouse('WebHID not available in this browser/context.');
            return;
        }
        if (!this._spaceMouse) this._initSpaceMouse();
        const sm = this._spaceMouse;
        if (!sm) return;
        try {
            const devices = await navigator.hid.requestDevice({ filters: sm.filters });
            logSpaceMouse('Device picker returned', Array.isArray(devices) ? devices.length : 0, 'devices');
            try {
                (devices || []).forEach((d, idx) => logSpaceMouse(`Picker device[${idx}]`, describeHidDevice(d)));
            } catch { }
            if (!devices || devices.length === 0) return;
            const device = devices.find(isSpaceMouseDevice) || devices[0];
            logSpaceMouse('Selected device', describeHidDevice(device));
            await this._connectSpaceMouseDevice(device);
            logSpaceMouse('Request complete.');
        } catch (error) {
            logSpaceMouse('Request failed', error);
        }
    }

    _initSpaceMouse() {
        if (this._spaceMouse || typeof navigator === 'undefined' || !navigator.hid) return;
        logSpaceMouse('Initializing WebHID listeners...');
        const sm = {
            supported: true,
            enabled: true,
            device: null,
            ...
            translationSign: new THREE.Vector3(1, 1, 1),
            rotationSign: new THREE.Vector3(1, 1, 1),
            buttons: 0,
            lastInputAt: 0,
            listening: false,
            lastReportId: null,
            lastReportLength: 0,
            _reportSeen: false,
            reportLogIntervalMs: 250,
            _lastReportLogAt: 0,
            deadzone: 8,
            maxInput: 350,
            translationSpeed: 0.6,
            rotationSpeed: 2.0,
            zoomSpeed: 0.9,
            filters: [
                { vendorId: 0x046d },
            ...
            _handleInputReport: null,
            _handleConnect: null,
            _handleDisconnect: null,
        };
        sm._handleInputReport = (e) => {
            if (!e || !e.data) return;
            const dv = e.data;
            sm.lastReportId = e.reportId;
            sm.lastReportLength = dv.byteLength;
            if (!sm._reportSeen) {
                sm._reportSeen = true;
                logSpaceMouse('First input report', e.reportId, dv.byteLength);
            }
            switch (e.reportId) {
                case 0: { // combined report on some devices
                    if (dv.byteLength >= 6) {
                        sm.translation.set(
                            dv.getInt16(0, true),
                            dv.getInt16(2, true),
            ...
            if (!sm._lastReportLogAt || (now - sm._lastReportLogAt) >= sm.reportLogIntervalMs) {
                sm._lastReportLogAt = now;
                logSpaceMouse('Input report', {
                    reportId: e.reportId,
                    byteLength: dv.byteLength,
                    translation: sm.translation.toArray(),
                    rotation: sm.rotation.toArray(),
                    buttons: sm.buttons,
                });
            }
            sm.lastInputAt = (typeof performance !== 'undefined') ? performance.now() : Date.now();
        };
        sm._handleConnect = (e) => {
            const device = e && e.device;
            logSpaceMouse('HID connect event', describeHidDevice(device));
            if (!isSpaceMouseDevice(device)) return;
            this._connectSpaceMouseDevice(device);
        };
        sm._handleDisconnect = (e) => {
            const device = e && e.device;
            logSpaceMouse('HID disconnect event', describeHidDevice(device));
            if (!sm.device || !device) return;
            if (device === sm.device || device.deviceId === sm.device.deviceId) {
                try { device.removeEventListener('inputreport', sm._handleInputReport); } catch { }
                try { device.oninputreport = null; } catch { }
                sm.device = null;
                sm.listening = false;
                sm.translation.set(0, 0, 0);
        ...
        this._spaceMouse = sm;
        logSpaceMouse('Registering navigator.hid connect/disconnect listeners');
        try { navigator.hid.addEventListener('connect', sm._handleConnect); } catch { }
        try { navigator.hid.addEventListener('disconnect', sm._handleDisconnect); } catch { }
        try {
            navigator.hid.getDevices().then((devices) => {
                logSpaceMouse('Existing HID devices', Array.isArray(devices) ? devices.length : 0);
                try {
                    (devices || []).forEach((d, idx) => logSpaceMouse(`Existing device[${idx}]`, describeHidDevice(d)));
                } catch { }
                if (!Array.isArray(devices) || devices.length === 0) return;
                const existing = devices.find(isSpaceMouseDevice);
                if (existing) this._connectSpaceMouseDevice(existing);
            }).catch(() => { });
        } catch { }
    }

    async _connectSpaceMouseDevice(device) {
        const sm = this._spaceMouse;
        if (!sm || !device) return;
        logSpaceMouse('Connecting to device', describeHidDevice(device));
        if (sm.device === device && sm.listening && device.opened) return;
        if (sm.device && sm.device !== device) {
            try { sm.device.removeEventListener('inputreport', sm._handleInputReport); } catch { }
            try { if (sm.device.opened) await sm.device.close(); } catch { }
        }
        sm.device = device;
        try {
            if (!device.opened) await device.open();
            logSpaceMouse('Device opened', describeHidDevice(device));
        } catch (error) {
            logSpaceMouse('Failed to open device', error);
            sm.device = null;
            return;
        }
        if (!this._spaceMouse || sm !== this._spaceMouse) return;
        try { device.addEventListener('inputreport', sm._handleInputReport); } catch { }
        try { device.oninputreport = sm._handleInputReport; } catch { }
        sm.listening = true;
        logSpaceMouse('Input report listeners attached');
    }

    _applySpaceMouseInput(dt) {
        const sm = this._spaceMouse;
        if (!sm || !sm.enabled || !sm.device || !this.controls || !this.controls.enabled) return;
        if (!this.camera || !this.renderer || !Number.isFinite(dt) || dt <= 0) return;

        const rawT = sm.translation;
        const rawR = sm.rotation;
        const tx = normalizeSpaceMouseAxis(rawT.x, sm.deadzone, sm.maxInput) * sm.translationSign.x;
        const ty = normalizeSpaceMouseAxis(rawT.y, sm.deadzone, sm.maxInput) * sm.translationSign.y;
        const tz = normalizeSpaceMouseAxis(rawT.z, sm.deadzone, sm.maxInput) * sm.translationSign.z;
        const rx = normalizeSpaceMouseAxis(rawR.x, sm.deadzone, sm.maxInput) * sm.rotationSign.x;
        const ry = normalizeSpaceMouseAxis(rawR.y, sm.deadzone, sm.maxInput) * sm.rotationSign.y;
        const rz = normalizeSpaceMouseAxis(rawR.z, sm.deadzone, sm.maxInput) * sm.rotationSign.z;

        if (!tx && !ty && !tz && !rx && !ry && !rz) return;

        const { right, up, forward, axis, move, tMat, size } = sm._tmp;
        ...
        if (this.camera.isOrthographicCamera) {
            const zoom = (typeof this.camera.zoom === 'number' && this.camera.zoom > 0) ? this.camera.zoom : 1;
            viewHeight = Math.abs(this.camera.top - this.camera.bottom) / zoom;
        } else {
            try {
                this.renderer.getSize(size);
                const wpp = this._worldPerPixel(this.camera, size.width, size.height);
                if (Number.isFinite(wpp) && wpp > 0) viewHeight = wpp * size.height;
            } catch { }
        }
        if (!Number.isFinite(viewHeight) || viewHeight <= 0) viewHeight = 1;

        const panSpeed = viewHeight * sm.translationSpeed;
        let didTransform = false;
        if (tx || ty) {
            move.set(0, 0, 0);
            move.addScaledVector(right, tx * panSpeed * dt);
            move.addScaledVector(up, ty * panSpeed * dt);
            if (move.lengthSq() > 0) {
                this.controls.updateMatrixState();
                tMat.makeTranslation(move.x, move.y, move.z);
                this.controls.applyTransformMatrix({ camera: tMat, gizmos: tMat });
                didTransform = true;
            }
        }

        if (rx || ry || rz) {
            axis.copy(right).multiplyScalar(rx);
            axis.addScaledVector(up, ry);
            axis.addScaledVector(forward, rz);
            const axisLen = axis.length();
            if (axisLen > 1e-6) {
                axis.divideScalar(axisLen);
                const angle = axisLen * sm.rotationSpeed * dt;
                this.controls.updateMatrixState();
                this.controls.applyTransformMatrix(this.controls.rotate(axis, angle));
                try {
                    if (this.controls._upState) {
                        this.camera.up.copy(this.controls._upState).applyQuaternion(this.camera.quaternion);
                    }
                } catch { }
                didTransform = true;
            }
        ...
        if (tz) {
            const zoomFactor = Math.exp(-tz * sm.zoomSpeed * dt);
            if (Number.isFinite(zoomFactor) && zoomFactor > 0 && zoomFactor !== 1) {
                const pivot = this.controls._gizmos?.position;
                if (pivot) {
                    this.controls.updateMatrixState();
                    this.controls.applyTransformMatrix(this.controls.scale(zoomFactor, pivot));
                    didTransform = true;
                }
            }
        }
        ...
        if (didTransform) {
            try { this.controls.updateMatrixState(); } catch { }
            try { this.controls.dispatchEvent({ type: 'change' }); } catch { }
        }
    }
    ...
    _disposeSpaceMouse() {
        const sm = this._spaceMouse;
        if (!sm) return;
        logSpaceMouse('Disposing SpaceMouse listeners');
        try {
            if (sm.device && sm._handleInputReport) {
                sm.device.removeEventListener('inputreport', sm._handleInputReport);
            }
        } catch { }
        try { if (sm.device) sm.device.oninputreport = null; } catch { }
        sm.listening = false;
        try { if (sm.device?.opened) sm.device.close(); } catch { }
        try {
            if (typeof navigator !== 'undefined' && navigator.hid) {
                if (sm._handleConnect) navigator.hid.removeEventListener('connect', sm._handleConnect);
                if (sm._handleDisconnect) navigator.hid.removeEventListener('disconnect', sm._handleDisconnect);
            }
        } catch { }
        this._spaceMouse = null;
    }
    ...
    dispose() {
        ...
        try { this._disposeSpaceMouse?.(); } catch { }
    ...
    _loop() {
        ...
        const now = (typeof performance !== 'undefined') ? performance.now() : Date.now();
        const prev = (typeof this._lastFrameAt === 'number') ? this._lastFrameAt : now;
        const dt = Math.min(0.05, Math.max(0, (now - prev) / 1000));
        this._lastFrameAt = now;
        if (this._spaceMouse) {
            try { this._applySpaceMouseInput(dt); } catch { }
        }
