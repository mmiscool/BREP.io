...
type SnapshotObject = AnyRecord | null | undefined;
type PersistentData = AnyRecord | null | undefined;
type ReferenceSnapshot = AnyRecord;
type ReferenceSnapshotBucket = Record<string, ReferenceSnapshot>;
type BuildReferenceSnapshotOptions = {
  sourceUuid?: unknown;
  sourceFeatureId?: unknown;
  sourceTimestamp?: unknown;
};
type CaptureReferenceSelectionOptions = {
  stores?: PersistentData[];
  schema?: AnyRecord | null;
  resolvedParams?: AnyRecord | null;
};

function normalizeReferenceSnapshotFieldKey(raw: unknown): string {
  const key = String(raw || '').trim();
  return key || DEFAULT_FIELD_KEY;
}

export function normalizeReferenceSnapshotName(obj: SnapshotObject): string | null {
  if (!obj) return null;
...
function getReferenceSnapshotStore(persistentData: PersistentData): AnyRecord | null {
  const store = persistentData?.[REFERENCE_SNAPSHOT_STORE_KEY];
  return store && typeof store === 'object' ? store : null;
}

export function ensureReferenceSnapshotBucket(
  persistentData: PersistentData,
  fieldKey: unknown,
): ReferenceSnapshotBucket | null {
...
export function getReferenceSnapshotBucket(persistentData: PersistentData, fieldKey: unknown): ReferenceSnapshotBucket | null {
...
export function extractEdgeWorldPositions(obj: SnapshotObject): number[] {
  if (!obj) return [];
...
function extractFaceEdgePositions(face: SnapshotObject): number[][] {
  if (!face) return [];
  const out: number[][] = [];
  const addEdge = (edge: SnapshotObject) => {
...
function extractFaceCenterNormal(
  face: SnapshotObject,
  edgePositions: number[][] | null = null,
): { center: number[] | null; normal: number[] | null } | null {
  if (!face) return null;
  try { face.updateMatrixWorld?.(true); } catch { /* ignore */ }

  let centerVec: THREE.Vector3 | null = null;
  try {
...
function getOwningFeatureIdForSnapshotObject(obj: SnapshotObject): unknown {
  let cur = obj;
  let guard = 0;
  while (cur && guard < 8) {
...
export function buildReferenceSnapshot(
  obj: SnapshotObject,
  options: BuildReferenceSnapshotOptions = {},
...
export function isReferenceSnapshotUsable(snapshot: ReferenceSnapshot | null | undefined): boolean {
...
function normalizeAllowedTypes(allowedTypes: string[] | Set<string> | null = null): Set<string> | null {
...
export function resolveReferenceSnapshotFromNames(
  persistentData: PersistentData,
  fieldKey: unknown,
  names: unknown[] = [],
  allowedTypes: string[] | Set<string> | null = null,
): ReferenceSnapshot | null {
  const bucket = getReferenceSnapshotBucket(persistentData, fieldKey);
  if (!bucket) return null;
  const typeSet = normalizeAllowedTypes(allowedTypes);
  const acceptsType = (snapshot: ReferenceSnapshot | null | undefined) => {
...
export function captureReferenceSelectionSnapshots({
  stores = [],
  schema = null,
  resolvedParams = null,
}: CaptureReferenceSelectionOptions = {}): number {
