...
function ensureFaceTriangles(face) {
...
function computeFaceEntry(face, options = {}) {
  const triangles = ensureFaceTriangles(face);
  ...
  return {
    face,
    faceName: String(face?.name || face?.userData?.faceName || '').trim() || face?.uuid || 'FACE',
    triangles,
    normal: avgNormal,
    point,
...
function collectSolidFaceEntries(solid, options = {}) {
  ...
  for (const child of children) {
    ...
    const entry = computeFaceEntry(child, options);
    if (entry) {
      faceEntries.push({
        ...entry,
        solid,
...
function analyzeFaceEntryPairs(faceEntriesA, faceEntriesB, options = {}, { sameSet = false } = {}) {
  const overlaps = [];
