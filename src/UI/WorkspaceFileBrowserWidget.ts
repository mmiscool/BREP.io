...
function formatSavedAt(savedAt) {
  ...
  return isNaN(dt.getTime()) ? 'Unknown time' : dt.toLocaleString();
}
...
export class WorkspaceFileBrowserWidget {
  ...
  constructor({
    ...
    this.iconSize = clampIconSize(readUiPreference(EXPLORER_ICON_SIZE_PREF_KEY, String(EXPLORER_ICON_SIZE_DEFAULT)));
  ...
  setData({ records = [], folders = [], roots = [] }: any = {}, { render = true }: any = {}) {
  ...
  setSearchTerm(value = '', { render = true, syncInput = true }: any = {}) {
  ...
  setViewMode(mode, { persist = true }: any = {}) {
  ...
  setLocation(location: any = {}, { persist = true }: any = {}) {
  ...
  _setViewMode(mode, { persist = true }: any = {}) {
  ...
  async _emptyTrashDirect(files = [], folders = []) {
    ...
    for (const entry of files) {
      ...
      const scope: any = { source, repoFull, path };
  ...
  _isFileDragEvent(event) {
    ...
    if (dt.items && dt.items.length) {
      return (Array.from(dt.items) as any[]).some((item) => item?.kind === 'file');
    }
