...
export class FeatureDimensionOverlay {
  ...
  constructor({ viewer = null, onFieldChange = null, onFieldFocus = null }: any = {}) {
    ...
    if (this.viewer?.scene) {
      ...
      try { this._group.userData.featureDimensionOverlay = true; } catch { /* ignore best-effort overlay failure */ }
    ...
    this._labelOverlay = new LabelOverlay(
      ...
      null,
      null,
      (idx, ann, ev) => { try { this.#handleLabelClick(idx, ann, ev); } catch { /* ignore best-effort overlay failure */ } },
      null,
    );
    try { this._labelOverlay?.setVisible?.(false); } catch { /* ignore best-effort overlay failure */ }

    try { this.viewer?.controls?.addEventListener?.('change', this._onControlsChange); } catch { /* ignore best-effort overlay failure */ }
    try { window.addEventListener('resize', this._onWindowResize); } catch { /* ignore best-effort overlay failure */ }
    try { this.viewer?.renderer?.domElement?.addEventListener?.('pointerdown', this._onCanvasPointerDown, true); } catch { /* ignore best-effort overlay failure */ }
  }

  dispose() {
    this.clearActive();
    try { this.viewer?.controls?.removeEventListener?.('change', this._onControlsChange); } catch { /* ignore best-effort overlay failure */ }
    try { window.removeEventListener('resize', this._onWindowResize); } catch { /* ignore best-effort overlay failure */ }
    try { this.viewer?.renderer?.domElement?.removeEventListener?.('pointerdown', this._onCanvasPointerDown, true); } catch { /* ignore best-effort overlay failure */ }
    this.#endDragSession();
    this.#cancelQueuedFieldChange();

    try { this._labelOverlay?.dispose?.(); } catch { /* ignore best-effort overlay failure */ }
    this._labelOverlay = null;

    if (this._group && this.viewer?.scene) {
      allowSceneOverlayRemoval(this._group, { deep: true });
      try { this.viewer.scene.remove(this._group); } catch { /* ignore best-effort overlay failure */ }
    }
    this._group = null;
  }
  ...
  clearActive() {
    ...
    this.#cancelQueuedFieldChange();
    this.#clearVisuals();
    try { this._labelOverlay?.clear?.(); } catch { /* ignore best-effort overlay failure */ }
    try { this._labelOverlay?.setVisible?.(false); } catch { /* ignore best-effort overlay failure */ }
    this.#requestRender();
  }
  ...
  setSuppressed(suppressed = false) {
    ...
    try { this._labelOverlay?.setVisible?.(!!this._active); } catch { /* ignore best-effort overlay failure */ }
    if (this._active) this.refresh();
  }

  setActive({ entryId = null, entry = null, featureClass = null, form = null }: any = {}) {
    ...
    try { this._labelOverlay?.setVisible?.(!this._suppressed); } catch { /* ignore best-effort overlay failure */ }
  }

  refresh({ preserveExistingOnEmpty = false }: any = {}) {
    if (!this._active) {
      this.clearActive();
      return;
    }
    ...
    if (!this._group.parent) {
      try { this.viewer.scene.add(this._group); } catch { /* ignore best-effort overlay failure */ }
    }
    ...
    for (const ann of annotations) {
      ...
      try { this._labelOverlay?.updateLabel?.(ann.id, ann.text, labelPosition, labelData); } catch { /* ignore best-effort overlay failure */ }
      ...
      if (labelEl) {
        ...
        } catch { /* ignore best-effort overlay failure */ }
      }
  ...
  #handleCanvasPointerDown(event) {
    ...
    try { if (controls) controls.enabled = false; } catch { /* ignore best-effort overlay failure */ }
    this.#refreshViewerHover(event);

    try { window.addEventListener('pointermove', this._onGlobalPointerMove, true); } catch { /* ignore best-effort overlay failure */ }
    try { window.addEventListener('pointerup', this._onGlobalPointerUp, true); } catch { /* ignore best-effort overlay failure */ }
  }
  ...
  #handleGlobalPointerUp(event) {
    ...
    try { window.removeEventListener('pointermove', this._onGlobalPointerMove, true); } catch { /* ignore best-effort overlay failure */ }
    try { window.removeEventListener('pointerup', this._onGlobalPointerUp, true); } catch { /* ignore best-effort overlay failure */ }

    try { if (state.controls) state.controls.enabled = state.prevControlsEnabled; } catch { /* ignore best-effort overlay failure */ }
    this._dragState = null;
  }
  ...
  #applyDraggedValue(annotation, value, commit) {
    ...
    if (exprMap && Object.prototype.hasOwnProperty.call(exprMap, annotation.fieldKey)) {
      try { delete exprMap[annotation.fieldKey]; } catch { /* ignore best-effort overlay failure */ }
    }

    try { this._active.form?.refreshFromParams?.(); } catch { /* ignore best-effort overlay failure */ }
  ...
  #upsertLines(annotationId, segments, color = FEATURE_LINE_COLOR) {
    ...
    if (!line) {
      ...
      try { this._group.add(line); } catch { /* ignore best-effort overlay failure */ }
      this._lineMap.set(annotationId, line);
    } else if (line.material) {
      try { line.material.color?.set?.(color); } catch { /* ignore best-effort overlay failure */ }
      ...
      try { line.material.needsUpdate = true; } catch { /* ignore best-effort overlay failure */ }
    }
  ...
  #upsertArrows(annotation, specs) {
    ...
    if (!group) {
      ...
      try { this._group?.add(group); } catch { /* ignore best-effort overlay failure */ }
      this._arrowGroupMap.set(annotationId, group);
    }

    const children: any[] = Array.from(group.children || []);
    for (const child of children) {
      try { group.remove(child); } catch { /* ignore best-effort overlay failure */ }
      try { child.geometry?.dispose?.(); } catch { /* ignore best-effort overlay failure */ }
      try { child.material?.dispose?.(); } catch { /* ignore best-effort overlay failure */ }
    }
  ...
  #removeLine(annotationId) {
    ...
    if (!line) return;
    try { line.parent?.remove?.(line); } catch { /* ignore best-effort overlay failure */ }
    try { line.geometry?.dispose?.(); } catch { /* ignore best-effort overlay failure */ }
    try { line.material?.dispose?.(); } catch { /* ignore best-effort overlay failure */ }
  ...
  #removeArrows(annotationId) {
    ...
    try { group.parent?.remove?.(group); } catch { /* ignore best-effort overlay failure */ }
  ...
  #refreshViewerHover(event = null) {
    ...
    if (!nextEvent) return;
    try { viewer._lastPointerEvent = nextEvent; } catch { /* ignore best-effort overlay failure */ }
    try { viewer._updateHover(nextEvent); } catch { /* ignore best-effort overlay failure */ }
  }
  ...
  #requestRender() {
    try { this.viewer?.render?.(); } catch { /* ignore best-effort overlay failure */ }
  }
}
