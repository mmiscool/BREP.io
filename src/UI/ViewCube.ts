...
export class ViewCube {
   ...
   */
  constructor({ renderer, targetCamera, controls = null, size = 110, margin = 10, colors = null }: any = {}) {
    ...
    const toCssAndHex = (input) => {
      ...
      if (typeof input === 'number') {
        ...
        } catch { /* ignore CSS color parsing failures */ }
      }
  ...
  _reorientCamera(dir, _faceName = '') {
    ...
    if (controls && controls.updateMatrixState) {
      try { controls.updateMatrixState(); } catch { /* ignore controls state sync failures */ }
    }
