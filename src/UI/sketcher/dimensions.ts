...
const dbg = (...args) => { try { if (DIM_DEBUG || window.__SKETCH_DIM_DEBUG) console.log('[DIM]', ...args); } catch { /* ignore */ } };

function selectionItems(inst): AnyRecord[] {
  return Array.from(inst?._selection || []) as AnyRecord[];
}
...
function clearDims(inst) {
  ...
  const labels = Array.from(inst._dimRoot.querySelectorAll('.dim-label, .glyph-label')) as Node[];
  ...
  if (inst._dim3D) {
    while (inst._dim3D.children.length) {
      ...
      try { ch.geometry?.dispose(); ch.material?.dispose?.(); } catch { /* ignore */ }
    }
  }
}
...
export function renderDimensions(inst) {
  ...
  if (inst._suspendDimLabelRebuild) {
    try { _redrawDim3D(inst); } catch { /* ignore */ }
    return;
  }
  clearDims(inst);
  // Reset per-frame angle geometry cache used to center labels on arc midpoints
  try { inst._dimAngleGeom = new Map(); } catch { /* ignore */ }
  ...
  const mk = (c, text, world, planeOffOverride = null, noNudge = false) => {
    ...
    try { d.dataset.cid = String(c.id); } catch { /* ignore */ }
    ...
    (d.style as any).MozUserSelect = 'none';
    ...
    const isSel = selectionItems(inst).some(it => it.type === 'constraint' && it.id === c.id);
  ...
  for (const c of s.constraints || []) {
    ...
    const sel = selectionItems(inst).some(it => it.type === 'constraint' && it.id === c.id);
    ...
    if (c.type === '⟺' || c.type === POINT_LINE_DISTANCE_TYPE) {
      if (c.type === '⟺' && isRadialDimensionConstraint(c)) {
        ...
        if (!Number.isFinite(Number(offSaved?.du)) || !Number.isFinite(Number(offSaved?.dv))) {
          try { inst._dimOffsets.set(c.id, { du: radialOff.du, dv: radialOff.dv }); } catch { /* ignore */ }
        }
  ...
  try { drawConstraintGlyphs(inst, glyphConstraints); } catch { /* ignore */ }
}
...
function _redrawDim3D(inst, onlyCid = null) {
  try {
    ...
    const dimBaseColor = getConstraintBaseColor(inst);
    const P = (id) => (s.points || []).find((p) => p.id === id);
    const selSet = new Set(selectionItems(inst).filter(it => it.type === 'constraint').map(it => it.id));
...
function attachDimLabelEvents(inst, el, c, world) {
  ...
  el.addEventListener('click', (e) => {
    ...
    try { inst.toggleSelectConstraint?.(c.id); } catch { /* ignore */ }
    e.preventDefault(); e.stopPropagation(); try { e.stopImmediatePropagation(); } catch { /* ignore */ }
    dbg('click', { cid: c.id, type: c.type });
  });

  // Hover reflects in overlays/sidebar
  el.addEventListener('pointerenter', () => { try { inst.hoverConstraintFromLabel?.(c.id); } catch { /* ignore */ } });
  el.addEventListener('pointerleave', () => { try { inst.clearHoverFromLabel?.(c.id); } catch { /* ignore */ } });
  ...
  el.addEventListener('dblclick', async (e) => {
    ...
    const resumeSolver = () => {
      if (!resumed && pausedByPrompt) {
        try { solver.resume(); } catch { /* ignore */ }
        resumed = true;
      }
    };
    if (pausedByPrompt) {
      try { solver.pause('dim-edit'); } catch { /* ignore */ }
    }
    ...
    try {
      ...
      resumeSolver();
      try { solver?.solveSketch('full'); } catch { /* ignore */ }
      try { solver?.hooks?.updateCanvas?.(); } catch { /* ignore */ }
    } finally {
      resumeSolver();
    }
  });
  ...
  let pendingOff: DimOffset | null = null;
  let swallowPointerCycle = false;

  el.addEventListener('pointerdown', (e) => {
    if (inst?._tool === 'trim') {
      swallowPointerCycle = true;
      try { inst.deleteConstraintFromLabel?.(c.id, e); } catch { /* ignore */ }
      e.preventDefault(); e.stopPropagation(); try { e.stopImmediatePropagation(); } catch { /* ignore */ }
      return;
    }
    dragging = true; moved = false; pendingOff = null;
    try { inst._suspendDimLabelRebuild = true; inst._activeDimLabelDragId = c.id; } catch { /* ignore */ }
    try { inst._debugDragCID = c.id; } catch { /* ignore */ }
    ...
    try { if (inst.viewer?.controls) inst.viewer.controls.enabled = false; } catch { /* ignore */ }
  ...
  el.addEventListener('pointermove', (e) => {
    ...
    if (isRadialDimensionConstraint(c)) {
      ...
      try { inst._dimOffsets.set(c.id, toLabel); _redrawDim3D(inst, c.id); } catch { /* ignore */ }
    } else {
      ...
      if (ends) {
        ...
        try { inst._dimOffsets.set(c.id, { du: toLabel.du, dv: toLabel.dv }); _redrawDim3D(inst, c.id); } catch { /* ignore */ }
      } else if (c.type === '∠') {
        ...
        pendingOff = { ...toGeom };
        try { inst._dimOffsets.set(c.id, toGeom); _redrawDim3D(inst, c.id); } catch { /* ignore */ }
  ...
  const commitAndRefresh = () => {
    if (pendingOff) { try { inst._dimOffsets.set(c.id, pendingOff); dbg('commit', { cid: c.id, off: pendingOff }); } catch { /* ignore */ } pendingOff = null; }
    try { inst._solver?.hooks?.updateCanvas?.(); } catch { /* ignore */ }
    try { renderDimensions(inst); dbg('renderDimensions'); } catch { /* ignore */ }
  };
  ...
  el.addEventListener('pointerup', (e) => {
    ...
    try { if (inst.viewer?.controls) inst.viewer.controls.enabled = true; } catch { /* ignore */ }
    if (!hadPending) { pendingOff = computePendingFromEvent(e); hadPending = !!pendingOff; dbg('pointerup-computed', { cid: c.id, pendingOff }); }
    if (hadPending) {
      commitAndRefresh();
      e.preventDefault(); e.stopPropagation(); try { e.stopImmediatePropagation(); } catch { /* ignore */ }
    }
    try { inst._suspendDimLabelRebuild = false; inst._activeDimLabelDragId = null; } catch { /* ignore */ }
    try { inst._debugDragCID = null; } catch { /* ignore */ }
  });
