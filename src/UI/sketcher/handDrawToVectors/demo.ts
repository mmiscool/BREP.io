...
const canvas = requireElement("draw", HTMLCanvasElement);
const ctx = canvas.getContext("2d", { willReadFrequently: true });
if (!ctx) throw new Error("2D canvas context is unavailable.");
const svg = requireElement("output", SVGSVGElement);
const btnVectorize = requireElement("vectorize", HTMLButtonElement);
const btnClear = requireElement("clear", HTMLButtonElement);
const btnToolDraw = requireElement("tool-draw", HTMLButtonElement);
const btnToolErase = requireElement("tool-erase", HTMLButtonElement);
const status = requireElement("status", HTMLElement);
const confidence = requireElement("confidence", HTMLElement);
const tuningControls = requireElement("tuning-controls", HTMLElement);
const autoUpdate = requireElement("auto-update", HTMLInputElement);
const btnResetTuning = requireElement("reset-tuning", HTMLButtonElement);
...
function setActiveTool(nextTool) {
  ...
  btnToolDraw.setAttribute("aria-pressed", activeTool === "draw" ? "true" : "false");
  btnToolErase.setAttribute("aria-pressed", activeTool === "erase" ? "true" : "false");
...
const controlsByKey = new Map<string, any>();
...
function buildTuningControls() {
  ...
  tunables.forEach((item) => {
    ...
    number.value = String(item.value);
...
function runVectorize() {
  ...
  const shapes: any[] = vectorizeImageData(imageData, getTuningOptions()) as any[];
  const theme = getThemeColors();
