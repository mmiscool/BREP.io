...
export class SketchMode3D {
  ...
  open() {
    ...
    try {
      ...
      if (savedOffsets && typeof savedOffsets === "object") {
        ...
        for (const [k, v] of Object.entries(savedOffsets) as any[]) {
    ...
    if (this._tool !== "select" && e.button === 0) {
      ...
      if (pid != null) {
        ...
        if (this._tool === "circle") {
          ...
          if (
            (Array.from(this._selection) as any[]).filter((i) => i.type === "point")
              .length === 2
          ) {
  ...
  #mountSketchSidebar() {
    ...
    this._sidebarPrevChildren = (Array.from(host.children || []) as any[]).map((el) => ({
      el,
  ...
  #mountSolverSettingsUI() {
    ...
    continuousBtn.ontouchstart = (e) => {
      e.preventDefault();
      continuousBtn.onmousedown?.(e as any);
    };
  ...
  #refreshContextBar() {
    ...
    const items = Array.from(this._selection) as any[];
    const s = this._solver.sketchObject;
  ...
  #toggleSelection(item) {
    ...
    const existing = (Array.from(this._selection) as any[]).find(
      (s) => s.type + ":" + s.id === key,
    );
  ...
  #addGeometry(type, pointIds, templateGeo = null, opts: any = {}) {
    const s = this._solver?.sketchObject;
    if (!s) return null;
  ...
  #rebuildSketchGraphics() {
    ...
    const addCurveObject = (pts, geo, type) => {
      ...
      const sel = (Array.from(this._selection) as any[]).some(
    ...
    for (const p of s.points || []) {
      const selected = (Array.from(this._selection) as any[]).some(
  ...
  #updateHandleSizes() {
    ...
    for (const ch of this._sketchGroup.children) {
      if (ch?.userData?.kind === "point") {
        const isSelected = (Array.from(this._selection) as any[]).some(
