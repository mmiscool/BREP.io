...
export function drawConstraintGlyphs(inst, constraints) {
  ...
  const placeGlyphLabel = (c, text, u, v, colorHex) => {
    ...
    el.style.padding = '1px 6px';
      ...
      el.addEventListener('pointerdown', (e) => {
        try { if (inst.viewer?.controls) inst.viewer.controls.enabled = false; } catch { /* ignore controls disable failures */ }
        try { el.setPointerCapture(e.pointerId); } catch { /* ignore pointer capture failures */ }
        e.preventDefault(); e.stopPropagation();
      });
      el.addEventListener('pointerup', (e) => {
        try { el.releasePointerCapture(e.pointerId); } catch { /* ignore pointer release failures */ }
        try { if (inst.viewer?.controls) inst.viewer.controls.enabled = true; } catch { /* ignore controls restore failures */ }
        if (inst?._tool === 'trim') {
          try { inst.deleteConstraintFromLabel?.(c.id, e); } catch { /* ignore constraint delete handler errors */ }
        } else {
          try { inst.toggleSelectConstraint?.(c.id); } catch { /* ignore selection handler errors */ }
        }
        e.preventDefault(); e.stopPropagation();
      });
      el.addEventListener('pointerenter', () => { try { inst.hoverConstraintFromLabel?.(c.id); } catch { /* ignore hover handler errors */ } });
      el.addEventListener('pointerleave', () => { try { inst.clearHoverFromLabel?.(c.id); } catch { /* ignore hover clear handler errors */ } });

      inst._dimRoot.appendChild(el);
    } catch { /* ignore glyph label placement failures */ }
  };
  const rect = inst.viewer.renderer.domElement.getBoundingClientRect();
  ...
  for (const c of (constraints || [])) {
    ...
    const ids = Array.from(new Set<number>((c.points || []).map(Number))).sort((a, b) => a - b);
  ...
  const selSet = new Set(Array.from(inst._selection || []).filter((it: any) => it.type === 'constraint').map((it: any) => it.id));
  ...
  for (const [key, arr] of groups.entries()) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      try { inst._glyphCenters.set(c.id, { u: adj.u, v: adj.v }); } catch { /* ignore glyph center bookkeeping failures */ }
      ...
      } catch { /* ignore invisible glyph hit mesh failures */ }
      ...
      } catch { /* ignore glyph symbol placement failures */ }
    }
  }
}
