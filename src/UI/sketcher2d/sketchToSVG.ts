...
function resolveCurveResolution(options: any = {}) {
  return normalizeCurveResolution(options?.curveResolution ?? options?.resolution, null);
}

function resolveBezierSamples(options: any = {}, curveResolution = null) {
...
function buildPaths(sketch, options: any = {}, includeSamples = false) {
...
function normalizeBasis(options: any = {}) {
...
function buildPolylineRows(sketch, options: any = {}) {
...
export function sketchToSVGPaths(sketch, options: any = {}) {
...
export function sketchToSVG(sketch, options: any = {}) {
  const precision = Number.isFinite(Number(options.precision)) ? Number(options.precision) : 3;
...
export function sketchToDXF(sketch, options: any = {}) {
  const curveResolution = resolveCurveResolution(options);
  const bezierSamples = resolveBezierSamples(options, curveResolution);
...
export function sketchTo3DPolylines(sketch, options: any = {}) {
  const curveResolution = resolveCurveResolution(options);
  const bezierSamples = resolveBezierSamples(options, curveResolution);
