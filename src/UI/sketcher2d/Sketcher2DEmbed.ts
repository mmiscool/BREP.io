...
function normalizeSidebarExpanded(value: any, fallback = false) {
  if (value == null) return fallback;
  return value !== false;
}

function normalizeGridVisible(value: any, fallback = DEFAULT_GRID_VISIBLE) {
  if (value == null) return fallback;
...
function normalizeGridSpacing(value: any, fallback = DEFAULT_GRID_SPACING) {
...
export class Sketcher2DEmbed {
  ...
  async destroy() {
    ...
    } catch { /* ignore listener cleanup failure */ }
    try {
...
<html lang="en">
  ...
  #onMessage(event: any) {
    ...
    if (msg.type === "sketchChanged") {
      ...
      this._latestSketch = sketch;
      try { this._onChange && this._onChange(sketch, deepClone(msg.payload || {})); } catch { /* ignore consumer change callback failure */ }
      return;
    }
    ...
    if (msg.type === "sketchFinished") {
      const sketch = deepClone(msg?.payload?.sketch || null);
      this._latestSketch = sketch;
      try { this._onFinished && this._onFinished(sketch, deepClone(msg.payload || {})); } catch { /* ignore consumer finished callback failure */ }
      return;
    }

    if (msg.type === "sketchCancelled") {
      try { this._onCancelled && this._onCancelled(deepClone(msg.payload || {})); } catch { /* ignore consumer cancelled callback failure */ }
    }
  }
}
