...
export class PMIViewsWidget {
  ...
  constructor(viewer: any, { readOnly = false }: any = {}) {
    this.viewer = viewer;
  ...
  dispose() {
    if (typeof this._removeHistoryListener === 'function') {
      try { this._removeHistoryListener(); } catch { /* ignore */ }
    }
    this._removeHistoryListener = null;
  ...
  _buildLabelLayout(labels: any[], width: number, height: number, cssWidth: any = null, renderContext: any = null, { svgCentered = false }: any = {}) {
  ...
  _createExportRenderContext(viewport: any, view: any = null, options: any = {}): any {
  ...
  _renderList() {
    ...
    views.forEach((v, idx) => {
      ...
      if (!this._readOnly) {
        ...
        const makeMenuButton = (label, handler, opts: any = {}) => {
        ...
        row.addEventListener('dblclick', (e) => {
          ...
          if ((target && menu.contains(target)) || target === menuBtn || tagName === 'INPUT') return;
          this._applyView(v, { index: idx });
        });
      } else {
  ...
  _toggleRowMenu(menu, trigger) {
    ...
    this._menuOutsideHandler = (evt) => {
      ...
      if (target && (this._activeMenu.contains(target) || trigger.contains(target))) return;
      this._closeActiveMenu();
    };
    setTimeout(() => {
  ...
  async _buildExportAnnotations(view, renderContext = null) {
    ...
    try { history.load(Array.isArray(view?.annotations) ? view.annotations : []); } catch { /* ignore */ }
  ...
  _alignNormal(alignment, ann, renderContext = null) {
    ...
    try { (renderContext?.viewer?.camera || this.viewer?.camera)?.getWorldDirection?.(n); } catch { /* ignore */ }
  ...
  _loadImageFromDataUrl(dataUrl): Promise<HTMLImageElement> {
  ...
  _applyView(view, { index = null, suppressActive = false, viewport = null } = {}) {
    try {
      ...
      if (!suppressActive && Number.isInteger(index)) {
        ...
        this._renderList();
        if (this._readOnly) {
          try { this.viewer?.startPMIPreviewMode?.(view, index, this); } catch { /* ignore */ }
        }
      }
    } catch { /* ignore */ }
  }
  ...
  async _enterEditMode(view, index) {
    if (this._readOnly) {
      try { this._applyView(view, { index }); } catch { /* ignore */ }
      return;
    }
    ...
    try { this._applyView(view, { index }); } catch { /* ignore */ }
    try { this.viewer.startPMIMode?.(view, index, this, { fromViewClick: true }); } catch { /* ignore */ }
  }
