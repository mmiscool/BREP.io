...
export class BaseAnnotation extends ListEntityBase {
  ...
  declare inputParams: any;
  declare persistentData: AnyRecord;
  declare runtimeAttributes: AnyRecord;
  declare renderingContext: AnyRecord | null;
  resultArtifacts: any[];
   ...
   */
  static dragLabelOnPlane(pmimode, ctx, options: AnyRecord = {}) {
    ...
    const onUp = (ev) => {
      try { target.removeEventListener('pointermove', onMove, true); } catch { /* ignore */ }
      try { target.removeEventListener('pointerup', onUp, true); } catch { /* ignore */ }
      if (options.suspendControls === true) {
        try { if (viewer?.controls) viewer.controls.enabled = true; } catch { /* ignore */ }
      }
      try { pmimode?.hideDragPlaneHelper?.(); } catch { /* ignore */ }
      if (typeof options.onEnd === 'function') {
        try { options.onEnd(ev); } catch { /* ignore */ }
      }
      if (options.preventDefault !== false) {
        try { ev.preventDefault(); ev.stopImmediatePropagation?.(); ev.stopPropagation(); } catch { /* ignore */ }
      }
    };

    try { pmimode?.showDragPlaneHelper?.(plane); } catch { /* ignore */ }
    if (options.suspendControls === true) {
      try { if (viewer?.controls) viewer.controls.enabled = false; } catch { /* ignore */ }
    }

    try { target.addEventListener('pointermove', onMove, true); } catch { /* ignore */ }
    try { target.addEventListener('pointerup', onUp, true); } catch { /* ignore */ }
  }
...
function sanitizeAnnotationParams(schema, rawParams, ann) {
  const sanitized = {};
  const params: AnyRecord = rawParams && typeof rawParams === 'object' ? rawParams : {};
