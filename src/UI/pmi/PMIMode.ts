...
export class PMIMode {
   ...
   */
  constructor(viewer: any, viewEntry: any, viewIndex: any, pmiWidget: any, options: any = {}) {
    this.viewer = viewer;
    ...
    try {
      for (const entity of this._annotationHistory.getEntries()) {
        try { this.#normalizeAnnotation(entity.inputParams); } catch { /* ignore */ }
  ...
  open() {
    ...
    try { v.scene.add(this._annGroup); } catch { /* ignore */ }
    ...
    if (!this._displayOnly) {
      ...
      try { v.renderer.domElement.addEventListener('pointerdown', this._onCanvasDown, { passive: false, capture: true }); } catch { /* ignore */ }
    }
    ...
    try {
      if (v.controls && typeof this._onControlsChange === 'function') {
        ...
        if (typeof v.controls.addEventListener === 'function') {
          try { v.controls.addEventListener('end', this._onControlsChange); } catch { /* ignore */ }
        }
      }
    } catch { /* ignore */ }
    ...
    if (!this._displayOnly) {
      try { if (v.controls) v.controls.enabled = true; } catch { /* ignore */ }
    }
  }
  ...
  #clearActiveReferenceSelection() {
    try {
      ...
      active.forEach((el) => {
        try { el.style.filter = 'none'; } catch { /* ignore */ }
        try { el.removeAttribute('active-reference-selection'); } catch { /* ignore */ }
        try {
    ...
    try { if (window.__BREP_activeRefInput) window.__BREP_activeRefInput = null; } catch { /* ignore */ }
    try { SelectionFilter.restoreAllowedSelectionTypes(); } catch { /* ignore */ }
  }
  ...
  async finish() {
    ...
    try { this.#_persistView(true); } catch { /* ignore */ }
    // Immediately return scene solids to modeling state before we notify the viewer
    try { this.#restoreViewTransforms(); } catch { /* ignore */ }
    try { this.#resetSolidsToBaseMatrices(); } catch { /* ignore */ }
    try { this.viewer.onPMIFinished?.(this.viewEntry); } catch { /* ignore */ }
    await this.dispose();
  }
  ...
  async dispose() {
    ...
    try { v.renderer.domElement.removeEventListener('pointerdown', this._onCanvasDown, { capture: true }); } catch { /* ignore */ }
    // Remove controls change listeners
    try {
      if (v.controls && typeof this._onControlsChange === 'function') {
        v.controls.removeEventListener('change', this._onControlsChange);
        try { v.controls.removeEventListener('end', this._onControlsChange); } catch { /* ignore */ }
      }
    } catch { /* ignore */ }
    // Remove overlay UI
    try { this._uiTopRight?.remove(); } catch { /* ignore */ }
    try { this.viewer?.mainToolbar?.clearRightReserve?.(this._toolbarRightReserveKey); } catch { /* ignore */ }
    ...
    try { if (this._annGroup && this._annGroup.parent) this._annGroup.parent.remove(this._annGroup); } catch { /* ignore */ }
    this._annGroup = null;
    try { if (this._refreshTimer) clearInterval(this._refreshTimer); } catch { /* ignore */ } this._refreshTimer = null;
    try { this.hideDragPlaneHelper(); } catch { /* ignore */ }
    // Remove labels overlay and destroy feature UIs
    try { this._labelOverlay?.dispose?.(); } catch { /* ignore */ }
    this._labelOverlay = null;
    try { this._annotationWidget?.dispose?.(); } catch { /* ignore */ }
    ...
    if (!this._displayOnly) {
      try { if (this.viewer?.controls) this.viewer.controls.enabled = !!this._controlsEnabledPrev; } catch { /* ignore */ }
    }
  }
  ...
  #notifyViewMutated(refreshList = false) {
    ...
    if (refreshList) {
      try { this.pmiWidget?.refreshFromHistory?.(); } catch { /* ignore */ }
      try { this.pmiWidget?._renderList?.(); } catch { /* ignore */ }
    }
  }
  ...
  #hideOriginalSidebarSections() {
    try {
      ...
      const titles: any[] = Array.from(accordion.querySelectorAll('.accordion-title'));
      titles.forEach((title) => {
        ...
        if (titleText === 'Scene Manager') {
          try { v.accordion.expandSection?.('Scene Manager'); } catch { /* ignore */ }
          return;
        }
  ...
  async #removePMISections() {
    try {
      ...
      try {
        ...
        const allTitles: any[] = Array.from(accordion.querySelectorAll('.accordion-title'));
        const allContents: any[] = Array.from(accordion.querySelectorAll('.accordion-content'));
  ...
  _refreshOverlays() {
    ...
    requestAnimationFrame(() => {
      ...
      try { this.#rebuildAnnotationObjects(); } catch { /* ignore */ }
    });
  }
  ...
  refreshAnnotationsUI() {
    try { this._annotationWidget?.render(); } catch { /* ignore */ }
    try {
      this.#rebuildAnnotationObjects();
      this._annotationsDirty = false;
      this.#_persistView();
  ...
  #rebuildAnnotationObjects() {
    ...
    if (!group) return;
    try { this._labelOverlay?.clear?.(); } catch { /* ignore */ }
    ...
    const ctx = {
      ...
      updateLabel: (idx, text, worldPos, ann) => { try { this._labelOverlay?.updateLabel?.(idx, text, worldPos, ann); } catch { /* ignore */ } },
  ...
  _handlePointerDown(e) {
    ...
    try { const activeRef = document.querySelector('[active-reference-selection="true"],[active-reference-selection=true]'); if (activeRef) return; } catch { /* ignore */ }

    return;
  }

  #handleLabelClick(idx, ann, e) {
    try { e.preventDefault(); e.stopImmediatePropagation?.(); e.stopPropagation(); } catch { /* ignore */ }
  ...
  #startLabelDrag(idx, ann, e) {
    e.preventDefault(); e.stopImmediatePropagation?.(); e.stopPropagation();
    try { if (this.viewer?.controls) this.viewer.controls.enabled = false; } catch { /* ignore */ }
    ...
    try {
      ...
      if (handler && typeof handler.onLabelPointerDown === "function") {
        const ctx = {
          ...
          updateLabel: (i, text, worldPos, a) => { try { this._labelOverlay?.updateLabel?.(i, text, worldPos, a); } catch { /* ignore */ } },
    ...
    try { if (this.viewer?.controls) this.viewer.controls.enabled = true; } catch { /* ignore */ }
  }
  ...
  #_alignNormal(alignment, ann) {
    ...
    try {
      ...
      if (name) {
        ...
        if (obj) {
          ...
          try { obj.updateMatrixWorld(true); w.applyMatrix3(new THREE.Matrix3().getNormalMatrix(obj.matrixWorld)); } catch { /* ignore */ }
    ...
    try { this.viewer?.camera?.getWorldDirection?.(n); } catch { /* ignore */ }
