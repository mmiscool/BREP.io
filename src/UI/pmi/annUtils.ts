...
function manualArcballSync(camera, controls, targetArray, snapshot) {
  ...
  if (gizmos) {
    ...
    if (typeof controls.calculateTbRadius === 'function') {
      try { controls._tbRadius = controls.calculateTbRadius(camera); } catch { /* ignore */ }
    }
    if (typeof controls.makeGizmos === 'function') {
      try { controls.makeGizmos(targetVector, controls._tbRadius || 1); } catch { /* ignore */ }
    }
  }
...
export function makeOverlayDashedLine(a, b, color = undefined, options: AnyRecord = {}) {
  const style = getPMIStyle();
