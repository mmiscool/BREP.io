...
export class CombinedTransformControls extends THREE.Object3D {
  ...
  dispose() {
    try { this.domElement?.removeEventListener('pointerdown', this._onPointerDown); } catch { /* ignore missing DOM listener */ }
    try { window.removeEventListener('pointermove', this._onPointerMove); } catch { /* ignore missing DOM listener */ }
    try { window.removeEventListener('pointerup', this._onPointerUp, { capture: true }); } catch { /* ignore missing DOM listener */ }
  }

  getHelper() { return this; }
  getMode() { return this.mode; }
  setMode(mode: any) { this.mode = String(mode || 'translate'); }
  ...
  setDimensionToggleEnabled(enabled: any) {
  ...
  setSize(s: any) { this._sizeMultiplier = Number(s) || 1; this.update(); }
  resetSize() { this.setSize(this._defaultSizeMultiplier); }
  setCamera(camera: any, { resetSize = false, refresh = true }: any = {}) {
  ...
  attach(object: any) {
    ...
    if (this.target) {
      try { this.target.updateMatrixWorld(true); } catch { /* ignore non-Object3D targets */ }
      ...
      try { this.update(); } catch { /* ignore stale target update failures */ }
    }
  }
  ...
  _buildGizmo() {
    ...
    // Axis builders
    const axes: any[] = [];
    const addAxis = (axis: any, colorText: any, _spriteLabel: any = null) => {
    ...
    return {
      ...
      rotateGroups: Object.values(rot).map((item: any) => item.group),
      center,
      centerMaterial: mCenter,
    };
  }

  _makeTextSprite(text: any, color = '#ffffff') {
    const size = 256;
  ...
  _handlePointerDown(e: any) {
    ...
    try { this.updateMatrixWorld(true); } catch { /* ignore stale gizmo transform updates */ }
  ...
  _handlePointerMove(e: any) {
    ...
    if (handle.kind === 'translate') {
      ...
      } catch { /* ignore rotate handle visualization failures */ }
    }
