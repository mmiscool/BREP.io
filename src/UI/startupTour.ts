...
export class StartupTour {
  ...
  static markDone() {
    try { writeBrowserStorageValue(TOUR_STORAGE_KEY, TOUR_STORAGE_VALUE); } catch { /* ignore */ }
  }
  ...
  destroy() {
    ...
    try { this._overlay?.remove(); } catch { /* ignore */ }
    this._overlay = null;
    this._highlight = null;
    this._card = null;
    this._titleEl = null;
    ...
    this._skipBtn = null;
    this._closeBtn = null;
    try { this._demoMouse?.detachCursor?.(); } catch { /* ignore */ }
    this._demoMouse = null;
    this._cursor = null;
    this._cursorLabel = null;
  ...
  async _waitFor(predicate, {
    ...
    while (Date.now() - started <= timeoutMs) {
      this._assertStepToken(token);
      let value = null;
      try { value = predicate(); } catch { /* ignore */ }
  ...
  async _moveCursorToElement(element, options: any = {}) {
  ...
  async _typeIntoInput(inputEl, value, {
    ...
    try { inputEl.dispatchEvent(new Event('change', { bubbles: true, composed: true })); } catch { /* ignore */ }
  ...
  async _changeSelectValue(selectEl, value, {
    ...
    try { selectEl.dispatchEvent(new Event('input', { bubbles: true, composed: true })); } catch { /* ignore */ }
    try { selectEl.dispatchEvent(new Event('change', { bubbles: true, composed: true })); } catch { /* ignore */ }
  ...
  async _createFeatureFromHistoryMenu(featureLabel, token) {
    ...
    const findMenuItem = () => {
      const items = Array.from(historyShadow.querySelectorAll('.hc-menu-item')) as any[];
    ...
    try {
      ...
      try { liveAddButton.click?.(); } catch { /* ignore */ }
      menuItem = await this._waitFor(findMenuItem, {
        timeoutMs: 1800,
        token,
      });
    ...
    try {
      ...
      await this._pulseCursor(token);
      try { liveMenuItem.click?.(); } catch { /* ignore */ }
      await this._waitFor(getNewFeature, {
        timeoutMs: 12000,
        token,
      });
  ...
  async _pickSceneReference(objectName, {
    ...
    try { SelectionFilter.ensureSelectionHandlers?.(object, { deep: true }); } catch { /* ignore */ }
    try { SelectionFilter.setHoverObject?.(object, { ignoreFilter: true }); } catch { /* ignore */ }
  ...
  async _waitForFeatureDimensionHandle(entryId, fieldKey, token, role = 'end') {
    await this._waitFor(() => {
      try { this.viewer?.historyWidget?.revealEntry?.(entryId, { focus: false, scroll: true, notify: false }); } catch { /* ignore */ }
      try { this._getFeatureDimensionOverlay()?.refresh?.(); } catch { /* ignore */ }
      try { this.viewer?.render?.(); } catch { /* ignore */ }
  ...
  async _activateTransformFromFeatureDimensionHandle(entryId, fieldKey, token, role = 'start') {
    ...
    try { handle.mesh.getWorldPosition(handleWorld); } catch { /* ignore */ }
  ...
  async _dragActiveTransformAxisToValue(entryId, axis, targetValue, {
    ...
    try { handle.getWorldPosition(handleWorld); } catch { /* ignore */ }
  ...
  async _editCubeFeature(feature, token) {
    ...
    try { this.viewer?.zoomToFit?.(1.18); } catch { /* ignore */ }
    await this._sleep(220, token);
  ...
  async _positionCylinderFeature(feature, token) {
    ...
    try { this.viewer?.zoomToFit?.(1.18); } catch { /* ignore */ }
    try { SchemaForm.__stopGlobalActiveXform?.(); } catch { /* ignore */ }
    await this._sleep(180, token);
  ...
  async _prepareLiveModelingDemo(token) {
    ...
    try { viewer.setActiveWorkbench?.('MODELING', { queueHistorySnapshot: false }); } catch { /* ignore */ }
    try {
  ...
  async runLiveDemoBooleanSubtract() {
    ...
    try {
      ...
      try { viewer?.zoomToFit?.(1.14); } catch { /* ignore */ }
      await this._sleep(250, token);
    } catch (error) {
      if (error?.code === TOUR_CANCELLED || error?.message === TOUR_CANCELLED) return;
...
export function resetStartupTourFlag() {
  try { removeBrowserStorageValue(TOUR_STORAGE_KEY); } catch { /* ignore */ }
}
