...
export class SelectionState {
    ...
    static attach(obj, { deep = false } = {}) {
        ...
        const attachOne = (target) => {
            ...
            if (!selectedDesc || !selectedDesc.get || !(selectedDesc.get as any).__selectionState) {
                const getter: any = function () { return state.selected; };
                getter.__selectionState = true;
                const setter: any = function (v) {
                    const nv = !!v;
                    ...
                    if (debugMode) {
                        ...
                        } catch { /* ignore selection state fallback failures */ }
                    }
                    state.selected = nv;
            ...
            if (!hoveredDesc || !hoveredDesc.get || !(hoveredDesc.get as any).__selectionState) {
                const getter: any = function () { return state.hovered; };
                getter.__selectionState = true;
                const setter: any = function (v) {
                    const nv = !!v;
    ...
    static setHoverColor(color) {
        if (!color) return;
        try { SelectionState.hoverColor = String(color); } catch { /* ignore selection state fallback failures */ }
    }
    ...
    static _seedBaseMaterials(obj) {
        ...
        for (const target of targets) {
            ...
            if (!SelectionState._isUsableMaterial(target.userData.__defaultMaterial)) {
                try { delete target.userData.__defaultMaterial; } catch { /* ignore selection state fallback failures */ }
            }
            if (!SelectionState._isUsableMaterial(target.userData.__baseMaterial)) {
                try { delete target.userData.__baseMaterial; } catch { /* ignore selection state fallback failures */ }
            }
            ...
            if (ud.__selectedMat && ud.__selectedMat !== base) {
                if (Array.isArray(ud.__selectedMat)) {
                    for (const m of ud.__selectedMat) {
                        try { if (m && typeof m.dispose === 'function') m.dispose(); } catch { /* ignore selection state fallback failures */ }
                    }
                } else if (typeof ud.__selectedMat.dispose === 'function') {
                    ud.__selectedMat.dispose();
                }
            }
            try { delete ud.__selectedMat; } catch { /* ignore selection state fallback failures */ }
            try { delete ud.__selectedMatBase; } catch { /* ignore selection state fallback failures */ }
            try { delete ud.__selectedColor; } catch { /* ignore selection state fallback failures */ }
            try { delete ud.__selectedLinewidth; } catch { /* ignore selection state fallback failures */ }
        } catch { /* ignore selection state fallback failures */ }
    ...
    static _clearHover(target) {
        ...
        try { delete ud.__hoverMatApplied; } catch { /* ignore selection state fallback failures */ }
        try { delete ud.__hoverOrigMat; } catch { /* ignore selection state fallback failures */ }
        try { delete ud.__hoverMat; } catch { /* ignore selection state fallback failures */ }
    }
    ...
    static _clearInPlaceHoverColor(target, ud = target?.userData || {}) {
        ...
        try { delete ud.__hoverColorApplied; } catch { /* ignore selection state fallback failures */ }
        try { delete ud.__hoverColorRecords; } catch { /* ignore selection state fallback failures */ }
    }
    ...
    static _resolveColor(value) {
        ...
        if (typeof value?.getHexString === 'function') {
            try { return `#${value.getHexString()}`; } catch { /* ignore selection state fallback failures */ }
        }
        return null;
    }
    ...
    static _shouldTraceMaterials() {
        try {
            return !!(
                ...
                || (globalThis?.window as any)?.__BREP_TRACE_MATERIALS
            );
        } catch {
    ...
    static _installMaterialWatcher(target) {
        ...
        if ((desc?.get as any)?.__brepMaterialWatcher) {
            try { target.__brepMaterialWatcherInstalled = true; } catch { /* ignore selection state fallback failures */ }
            return true;
        }
        let current;
        try { current = target.material; } catch { return false; }
        const getter: any = function () { return current; };
        getter.__brepMaterialWatcher = true;
        const setter: any = function (next) {
            const prev = current;
            current = next;
