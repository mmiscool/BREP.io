...
function promisifyRequest(req: any) {
...
async function idbGetAllEntries(db: any) {
  const tx = db.transaction([VFS_STORE_NAME], 'readonly');
  const store = tx.objectStore(VFS_STORE_NAME);
...
async function idbPutDeleteEntries(db: any, puts: any, deletes: any) {
  const tx = db.transaction([VFS_STORE_NAME], 'readwrite');
  const store = tx.objectStore(VFS_STORE_NAME);
...
function strToUint8(str: any) { return new TextEncoder().encode(str); }
function uint8ToStr(uint8: any) { return new TextDecoder().decode(uint8); }
function uint8ToBase64(uint8: any) {
  let binary = '';
  const chunk = 0x8000;
...
function base64ToUint8(b64: any) {
  const binary = atob(b64);
  const out = new Uint8Array(binary.length);
  for (let i = 0; i < binary.length; i++) out[i] = binary.charCodeAt(i);
  return out;
}
let decodedEntryCache: any = null;
let decodedEntryCacheB64 = '';
let decodedEntryCacheBytes: any = null;
let decodedEntryCacheUtf8: any = null;

function getDecodedEntryCache(entry: any, b64: any) {
  if (
...
function setDecodedEntryCache(entry: any, b64: any, bytes: any, utf8: any = null) {
...
class IndexedDbFS {
  ...
  async _init() {
    ...
    try {
      ...
      if (legacyIndex) {
        ...
        const map: any = await idbGetAllEntries(db);
        if (map.size) {
          const entries: any = {};
  ...
  _enoent(code: any, path: any) { const err: any = new Error(`${code}: no such file or directory, ${path}`); err.code = code; return err; }
  _eexist(code: any, path: any) { const err: any = new Error(`${code}: file already exists, ${path}`); err.code = code; return err; }
  _linkIntoParent(p: any) {
  ...
  rmdirSync(path: any, options: any = {}) {
    ...
    if (entry.children.length && !recursive) {
      const err: any = new Error(`ENOTEMPTY: directory not empty, ${path}`); err.code = 'ENOTEMPTY'; throw err;
    }
    const toDelete = [];
    if (recursive) {
      const stack = [path];
  ...
  renameSync(oldPath: any, newPath: any) {
    ...
    if (this.index.entries[newPath]) {
      if (this.index.entries[newPath].type === 'dir') {
        const err: any = new Error(`EISDIR: illegal operation on a directory, ${newPath}`); err.code = 'EISDIR'; throw err;
      }
  ...
  readFile(...args: any[]) { return this._cbify(this.readFileSync, ...args); }
  writeFile(...args: any[]) { return this._cbify(this.writeFileSync, ...args); }
  appendFile(...args: any[]) { return this._cbify(this.appendFileSync, ...args); }
  mkdir(...args: any[]) { return this._cbify(this.mkdirSync, ...args); }
  readdir(...args: any[]) { return this._cbify(this.readdirSync, ...args); }
  unlink(...args: any[]) { return this._cbify(this.unlinkSync, ...args); }
  rmdir(...args: any[]) { return this._cbify(this.rmdirSync, ...args); }
  rename(...args: any[]) { return this._cbify(this.renameSync, ...args); }
  stat(...args: any[]) { return this._cbify(this.statSync, ...args); }

  get promises() {
    const wrap = (syncFn: any) => (...args: any[]) => {
  ...
  rmSync(path: any, options: any = {}) {
    ...
    if (path === '/') { const err: any = new Error(`EPERM: operation not permitted, rm '${path}'`); err.code = 'EPERM'; throw err; }
    const entry = this.index.entries[path];
...
const browserFs: any = (typeof window !== 'undefined') ? new IndexedDbFS() : null;

// -------------------- Public Export --------------------

const universalFs: any = {
