...
async function runFixture(fixture, fixturePath) {
    ...
    if (fixture.sketch && typeof fixture.sketch === 'object') {
        ...
        const raw = await (fs.promises as any).readFile(partPath, 'utf8');
...
export async function registerSketchSolverTopologyFixtureTests(testFunctions) {
    ...
    let entries = [];
    try {
        entries = await (fs.promises as any).readdir(FIXTURE_DIR);
    } catch {
        return 0;
    }
    ...
    for (const file of files) {
        ...
        let fixture = null;
        try {
            const raw = await (fs.promises as any).readFile(filePath, 'utf8');
