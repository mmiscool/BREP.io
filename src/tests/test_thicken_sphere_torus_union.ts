...
export async function afterRun_thicken_sphere_torus_union(partHistory) {
  ...
  const sphereFaceNames = Array.from(new Set<string>((sphereSolid.faces || [])
  ...
  const sidewallFaceNames = Array.from(new Set<string>((torusSideSolid.faces || [])
    .map((face) => String(face.name || ""))
  ...
  const torusFaceNames = Array.from(new Set<string>((torusSideSolid.faces || [])
    .map((face) => String(face.name || ""))
    .filter(Boolean)));
