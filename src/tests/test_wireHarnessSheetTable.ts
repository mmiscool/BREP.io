...
export async function test_wire_harness_sheet_table_insert() {
  ...
  const tableData = buildWireHarnessSheetTableData(connections, routeResults as any);
  ...
  const inserted = insertWireHarnessConnectionTable(manager, sheet.id, connections, routeResults as any);
