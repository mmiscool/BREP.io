...
export function test_transform_reference_sanitize_preserves_metadata() {
  ...
  const reference: any = sanitized.reference;
  assert(reference.name === 'FACE_REF', 'Expected transform reference name to persist.');
  assert(reference.type === 'FACE', 'Expected transform reference type to persist.');
  assert(Array.isArray(reference.pickPoint), 'Expected pick point metadata to persist.');
  assert(reference.faceIndex === 3, 'Expected face index metadata to persist.');
}
