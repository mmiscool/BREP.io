...
export class BrowserTesting {
  ...
  constructor({
    ...
    this.env = viewer instanceof Viewer ? viewer : new Viewer({ container: containerEl, sidebar: sidebarEl } as any);
    if (exposeEnvOnWindow) (window as any).env = this.env;
    ...
    (window as any).browserTesting = this;
  ...
  nextFrame() { return new Promise<void>(res => requestAnimationFrame(() => res())); }
  ...
  // ====== Visibility helpers ======
  show() { try { if (this.ui?.window?.root) this.ui.window.root.style.display = ''; } catch { /* ignore hidden test widget */ } }
  hide() { try { if (this.ui?.window?.root) this.ui.window.root.style.display = 'none'; } catch { /* ignore hidden test widget */ } }
  toggle() {
    try {
      if (!this.ui?.window?.root) return;
      const cur = this.ui.window.root.style.display;
      this.ui.window.root.style.display = (cur === 'none') ? '' : 'none';
    } catch { /* ignore hidden test widget */ }
  }
  ...
  _buildUI() {
    ...
    this.testNames.forEach((name) => {
      ...
      row.addEventListener("click", (e) => {
        ...
        if ((e.target as any)?.closest?.("button,input,label")) return;
  ...
  _scrollRowIntoView(name = this.testNames[this.currentIndex]) {
    ...
    if (scrollContainer) {
      ...
      } catch { /* ignore scroll visibility measurement failure */ }
    }

    try {
      row.scrollIntoView({ block: "nearest", inline: "nearest", behavior: "auto" });
    } catch {
      try { row.scrollIntoView(false); } catch { /* ignore legacy scroll failure */ }
    }
  }
  ...
  _ensureLogWindow() {
    ...
    try { fw.root.style.display = "none"; } catch { /* ignore hidden log window */ }
  }

  _hideLogWindow() {
    if (!this.logWindow?.root) return;
    try { this.logWindow.root.style.display = "none"; } catch { /* ignore hidden log window */ }
  }
  ...
  async _runSingleByName(name) {
    ...
    try {
      ...
      if (typeof runSingleTest === "function") {
        const skipReason = getTestSkipReason(functionToRun, { runtime: "browser" });
        if (skipReason) {
          this._setError(name, {
...
function getNowMs() {
  ...
  } catch { /* ignore performance timer access failure */ }
  return Date.now();
}
